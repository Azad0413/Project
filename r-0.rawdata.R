...
library(tidyverse)
## 读取从xena下载的数据
tcga.expr<-read_tsv(file = '/data/nas1/luchunlin/TCGA.matrix/TCGA-STAD.htseq_counts.tsv.gz')%>%as.data.frame()%>%
...
# 375
## 保留有生存数据的
survival<-read.delim2('/data/nas1/luchunlin/TCGA_survival/TCGA-STAD.survival.tsv')
...
##fpkm
expr_fpkm<-read_tsv(file = '/data/nas1/luchunlin/TCGA.matrix/TCGA-STAD.htseq_fpkm.tsv.gz')%>%as.data.frame()%>%
  column_to_rownames(var = 'Ensembl_ID')
## xena下载的数据经过了log2+1转化，需要将其还原
...
dat.va<-dat.va %>%
  ...
  tibble::column_to_rownames(colnames(.)[1])   ## 把第一列变成行名并删除
pd1<-pData(a)
write.table(dat.va,file = 'dat.va(GSE62254).xls',sep = '\t',quote = F,row.names = T)
pd <- read_xlsx('../08_External_va/pd(GSE62254).xlsx')%>%as.data.frame()
colnames(pd) <- c('sample','OS','OS.time')
##0 1 alive 2 3 4 dead
class(pd$sample)
pd$sample <- as.character(pd$sample)
pd$sample <- paste0('T',pd$sample)
rownames(pd) <- pd$sample
pd <- pd[pd1$title,]
pd1 <- pd1[c(1:2)]
pd <- merge(pd,pd1,by.x='sample',by.y='title')

survival.va<-data.frame(sample=pd$geo_accession,OS=pd$OS,OS.time=pd$OS.time)
survival.va$OS.time<-as.numeric(survival.va$OS.time)*30
survival.va <- na.omit(survival.va)
table(survival.va$OS)
# survival.va <- subset(survival.va,OS=='0'|OS=='1'|OS=='2'|OS=='3')
survival.va$OS <- gsub('1','0',survival.va$OS,fixed = T)

survival.va$OS <- ifelse(survival.va$OS=='0',0,1)
survival.va$OS<-as.numeric(survival.va$OS)
write.table(survival.va,file = 'survival.va(GSE62254).xls',sep = '\t',row.names = F,quote = F)
