...
if (! dir.exists("./08_expression(GSE122063)")){
  dir.create("./08_expression(GSE122063)")
}
setwd("./08_expression(GSE122063)")

hubgene <- read.delim2('../06_PPI/hubgene.xls')
dat<-read.delim2("../00_rawdata/dat(GSE122063).xls", row.names = 1)%>% lc.tableToNum
group = read.delim2("../00_rawdata/group(GSE122063).xls")
control.sample<-group$sample[which(group$group=='control')]
df<-read.delim2('../01_DEGs(GSE122063)/DEG_sig(GSE122063).xls')
## 
library(tidyverse)
...
## 样本分组
hub_exp2$Group<-ifelse(hub_exp2$sample%in%control.sample,'control','VaD')
