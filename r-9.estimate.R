...
scores<-read.delim2('/data/nas1/luchunlin/project/BJTC-258/05_survival/clinical.xls')
high.sample<-scores$sample[which(scores$group=='High')]
low.sample<-scores$sample[which(scores$group=='Low')]
...
es_cor_dat <- es_cor_dat[scores$sample,]
es_cor_dat$score <- scores$scores

es_cor_dat$group <- ifelse(rownames(es_cor_dat) %in% high.sample,
                           "High", "Low")
es_cor_dat$score<-as.numeric(es_cor_dat$score)
imm_cor <- ggplot(es_cor_dat, aes(x = score,
                                  y = ImmuneScore)) +
  ...
  theme(axis.text.x=element_text(family="Times",size=12,face="bold"), #设置x轴刻度标签的字体显示倾斜角度为15度，并向下调整1(hjust = 1)，字体簇为Times大小为20
        ...
        legend.position = "top")
imm_cor
stromal_cor <- ggplot(es_cor_dat, aes(x = score,
                                      y = StromalScore)) +
  geom_point(aes(x = score,
                 y = StromalScore,
                 color = group),
  ...
  theme(axis.text.x=element_text(family="Times",size=12,face="bold"), #设置x轴刻度标签的字体显示倾斜角度为15度，并向下调整1(hjust = 1)，字体簇为Times大小为20
        ...
        legend.position = "top")
stromal_cor
estimate_cor <- ggplot(es_cor_dat, aes(x = score,
                                       y = ESTIMATEScore)) +
  geom_point(aes(x = score,
                 y = ESTIMATEScore,
                 color = group),
  ...
  theme(axis.text.x=element_text(family="Times",size=12,face="bold"), #设置x轴刻度标签的字体显示倾斜角度为15度，并向下调整1(hjust = 1)，字体簇为Times大小为20
        ...
        legend.position = "top")
estimate_cor
purity_cor <- ggplot(es_cor_dat, aes(x = score,
                                     y = TumorPurity)) +
  geom_point(aes(x = score,
                 y = TumorPurity,
                 color = group),
  ...
  theme(axis.text.x=element_text(family="Times",size=12,face="bold"), #设置x轴刻度标签的字体显示倾斜角度为15度，并向下调整1(hjust = 1)，字体簇为Times大小为20
        ...
        legend.position = "top")
purity_cor
score_dis <- ggplot(es_cor_dat, aes(x = group, 
                                        y = score,
                                        fill = group)) +
...

{
  ...
  #   23#
  # "
  # imm_all <- score_dis + imm_cor2  + imm_dis + 
  #   plot_layout(design = layout)
  # imm_all
  imm_all <- score_dis + plot_spacer() + imm_cor2  + imm_dis +
    plot_layout(ncol = 2, heights = c(1,6), widths = c(8,1)) 
  # &
  #   theme(plot.margin = margin(t =-1, l = 1, r=-1, b =-1))
  imm_all
  write_fig(imm_all,
            file = "01.immune_all.pdf",
            width = 7,
            height = 6,
            devices = NULL,
            res = 600,
...

{
  stromal_cor2 <- stromal_cor + theme(legend.position = "none",
                                      plot.margin = margin(t = 0,
                                                           ...
                                                           r = 0))
  stromal_cor2
  stromal_all <- score_dis + plot_spacer() + stromal_cor2 + stromal_dis +
    plot_layout(ncol =2, heights = c(1,6), widths = c(8,1))
  stromal_all
  write_fig(stromal_all,
            file = "02.stromal_all.pdf",
            width = 7,
            height = 6,
            devices = NULL,
            res = 600,
...

{
  estimate_cor2 <- estimate_cor + theme(legend.position = "none",
                                        plot.margin = margin(t = 0,
                                                             ...
                                                             r = 0))
  estimate_cor2
  estimate_all <- score_dis + plot_spacer() + estimate_cor2 + estimate_dis +
    plot_layout(ncol =2, heights = c(1,6), widths = c(8,1))
  estimate_all
  write_fig(estimate_all,
            file = "03.estimate_all.pdf",
            width = 7,
            height = 6,
            devices = NULL,
            res = 600,
...

{
  purity_cor2 <- purity_cor + theme(legend.position = "none",
                                    plot.margin = margin(t = 0,
                                                         ...
                                                         r = 0))
  purity_cor2
  purity_all <- score_dis + plot_spacer() + purity_cor2 + purity_dis +
    plot_layout(ncol = 2, heights = c(1,6), widths = c(8,1))
  purity_all
  write_fig(purity_all,
            file = "04.purity_all.pdf",
            width = 7,
            height = 6,
            devices = NULL,
            res = 600,
...
{
  es_cor_all <- imm_all / purity_all | stromal_all / estimate_all
  es_cor_all
  write_fig(es_cor_all,
            file = "05.es_allplot.pdf",
            width = 12,
            height = 10,
            devices = NULL,
            res = 600,
