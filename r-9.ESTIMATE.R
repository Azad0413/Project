...
Low<-rownames(design)[which(design$`Low UGCG`==1)]
High<-rownames(design)[which(design$`High UGCG`==1)]
length(Low)
length(High)
...

{
  ...
  #   guides(fill='none')
  # p4
  p5 <- cowplot::plot_grid(p1,p2,
                           nrow = 1, 
                           align = 'h', 
                           vjust = -0.3)
  p5
}
ggsave(filename = '01.estimate.all.pdf',p5,w=8,h=4)
ggsave(filename = '01.estimate.all.png',p5,w=8,h=4)
