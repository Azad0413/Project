...
library(readr)
library(tidyverse)
survival<-read_tsv(file = 'TCGA-ESCA.survival.tsv')
survival<-survival[,-3]
dat.tcga<-read.delim2("/data/nas1/luchunlin/project/BJTC-308/00_rawdata/dat.fpkm.xls", row.names = 1)%>% lc.tableToNum
...
group<-read.delim2('/data/nas1/luchunlin/project/BJTC-308/01_DEGs/group.xls')
tumor.sample<-group$sample[which(group$group=='Tumor')]
dat<-dat.tcga[,tumor.sample]
##162
DENRG<-read.delim2('/data/nas1/luchunlin/project/BJTC-308/02_DENRG/DENRG.xls')
...
lasso_geneids
## 9
## [1]  "HSPH1"   "GLA"     "FABP3"   "GK"      "FMO1"    "PNPLA3"  "PLA2G2F" "CYP46A1" "PNPLA5" 
...
dat.va<-read.delim2('/data/nas1/luchunlin/project/BJTC-308/00_rawdata/dat.va.xls',row.names = 1)%>% lc.tableToNum
test_data<-t(dat.va)%>%as.data.frame()
#test_data<-log2(test_data+1)
test_data$sample<-rownames(test_data)
survival.va<-read.delim2('/data/nas1/luchunlin/project/BJTC-308/00_rawdata/survival.va.xls')
### GSM6956-7075是小队列  GSE7076-7313是大队列
survival.va<-survival.va[order(survival.va$sample),]
survival.va<-survival.va[c(1:60),]
test_data<-test_data[rownames(test_data)%in%survival.va$sample,]
...
for_multi_ROC <- multi_ROC(time_vector = c(365*seq(1,5,1)), 
