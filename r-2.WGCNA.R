...
## 1759 6614 3621 1159 1133  445  332  232  210  153  152  115  114  105  101   84   75   75   67   65   61   61   55   54   52   30 
## 灰色的为**未分类**到模块的基因。
...
diff<-read.delim2('/data/nas1/luchunlin/project/BJTC-302/01_DEGs/DEG_sig.xls',row.names = 1)
...
##2533
DEIMG<-DE.MOD[DE.MOD$.%in%immune$.,]%>%as.data.frame()
## 113
colnames(DEIMG)<-'symbol'
write.table(DEIMG,file = 'DEIMG.xls',sep = '\t',row.names = F,quote = F)
library(ggvenn)
...
# mydata<-list('DEGs'=rownames(diff),'Immune'=immune$.,'MOD'=modGenes$modgene)
# ggvenn(mydata,c('DEGs','Immune','MOD'),
...
#                yLabels = colnames(MEs_col)[-27], 
#                cex.lab = 1, 
#                ySymbols = colnames(MEs_col)[-27], 
#                colorLabels = FALSE, 
...
#                cex.text = 0.8, 
#                zlim = c(-1,1),
#                main = paste("Module-tbsa relationships"))
# 
# ## 04-7 筛选强相关性模块------------
# ## blue
# module='blue'
