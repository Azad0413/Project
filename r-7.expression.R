...
if (! dir.exists("./07_expression")){
  dir.create("./07_expression")
}
setwd("./07_expression")
dat<-read.delim2("../00_rawdata/dat(GSE27276).xls", row.names = 1)%>% lc.tableToNum
group = read.delim2("../00_rawdata/group(GSE27276).xls")
control.sample<-group$sample[which(group$group=='control')]
df<-read.delim2('../01_DEGs/DEG_sig(GSE27276).xls',row.names = 1)
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  stat_pvalue_manual(stat.test,
                     y.position = c(12,16,16,16,12,13,11,14),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
...
exp_plot
ggsave('01.expression(GSE27276).pdf',exp_plot,width = 7,height = 7)
ggsave('01.expression(GSE27276).png',exp_plot,width = 7,height = 7)


##----------

dat<-read.delim2("../00_rawdata/dat(GSE100927).xls", row.names = 1)%>% lc.tableToNum
group = read.delim2("../00_rawdata/group(GSE100927).xls")
control.sample<-group$sample[which(group$group=='control')]
df<-read.delim2('../01_DEGs/DEG_sig(GSE100927).xls',row.names = 1)
hubgene<-read.delim2('../06_PPI/hubgene.xls')
hub_exp<-dat[hubgene$symbol,]
hub_exp2<-hub_exp
hub_exp2$Symbol<-rownames(hub_exp2)
...
## 样本分组
hub_exp2$Group<-ifelse(hub_exp2$sample%in%control.sample,'control','AS')
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  labs(title="Expression", x="", y = "",size=20) +
  stat_pvalue_manual(stat.test,
                     y.position = c(8,14,16,8,8,10,9,7),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
  ...
  facet_wrap(~Symbol,scales = "free",nrow = 3) 
exp_plot
ggsave('02.expression(GSE100927).pdf',exp_plot,width = 7,height = 7)
ggsave('02.expression(GSE100927).png',exp_plot,width = 7,height = 7)
