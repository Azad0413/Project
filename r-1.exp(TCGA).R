...
if (! dir.exists("./01_exp(TCGA)")){
  dir.create("./01_exp(TCGA)")
}
setwd("./01_exp(TCGA)")

dat<-read.csv('../00_rawdata/dat.fpkm.xls',sep = '\t')
grps = ifelse(colnames(dat) %>% str_ends(".1.."),"normal","tumor")
group <- data.frame(sample=colnames(dat),group=grps)
table(group$group)
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  labs(title="Expression(TCGA)", x="", y = "",size=20) +
  stat_pvalue_manual(stat.test,
                     y.position = c(7,12,8.5,2.5,4.5,5,10),
                     size = 3.2,
                     family = "Times",
                     label = "p",
                     #parse = T,
...
exp_plot
ggsave('01.expression(TCGA).pdf',exp_plot,width = 8,height = 6)
ggsave('01.expression(TCGA).png',exp_plot,width = 8,height = 6)
