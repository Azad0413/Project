...
group<-data.frame(sample=colnames(dat),group=c(rep('Paroxysmal',5),rep('Persistent',5)))
write.table(group,file = 'group.xls',sep = '\t',row.names = F,quote = F)
Paroxysmal.sample <- group$sample[which(group$group=='Paroxysmal')]
group2 <- group
colnames(group)
...
compare<- makeContrasts("Persistent-Paroxysmal", levels = design)
...
p <- ggplot(data = dat_plot,aes(x = id,y = t,fill = threshold)) +
  geom_col()+
  coord_flip() +
  scale_fill_manual(values = c('Up'= '#FF0088','NoSignifi'='#cccccc','Down'='#7bcd7b')) +
...
p
ggsave(filename = '01.GSVA.pdf',p,w=14,h=6)
ggsave(filename = '01.GSVA.png',p,w=14,h=6)

##热图
group.gsva<-group2

heat<-es_KEGG[rownames(DEGeneSets),]
## 按照分组排一下序
...
ann_colors<-list(
  Group = c(Paroxysmal="#00FFFF",Persistent="#FFAEB9"))
library(pheatmap)
pdf(file = '02.heatmap.pdf',w=12,h=4)
pheatmap(heat,
...
png(file = '02.heatmap.png',w=1000,h=300)
pheatmap(heat,
         color = colorRampPalette(color.key)(50),
         border_color = NA,
         annotation_col = annotation_col,
