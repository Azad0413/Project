...
hubgene <- read.delim2('../11_validation(GSE138125)/hub.final.xls')
...
for (i in c(1:5)){
  ...
  KEGG<-GSEA(geneList,TERM2GENE = kegggmt,pvalueCutoff = 0.3) #GSEA分析
  sortKEGG<-data.frame(KEGG)
  sortKEGG<-sortKEGG[order(sortKEGG$p.adjust, decreasing = F),]#按照enrichment score从高到低排序
  write.table(sortKEGG,file=paste('0',i,'.',rownames(hub.exp)[i],'(GSE27276)','.xls',sep=''),sep="\t",quote=F,row.names=F)
  ...
  fn1 = paste0("0", i, ".", rownames(hub.exp)[i],'(GSE27276)', ".png")
  fn2 = paste0("0", i, ".", rownames(hub.exp)[i],'(GSE27276)', ".pdf")
  ggsave(fn1, p, width = 10, height = 6, units = "in", limitsize = 300)
  ggsave(fn2, p, width = 10, height = 6, units = "in", limitsize = 300)
  i<-i+1
}
...
for (i in c(1:5)){
  ...
  sortKEGG<-data.frame(KEGG)
  sortKEGG<-sortKEGG[order(sortKEGG$p.adjust, decreasing = F),]#按照enrichment score从高到低排序
  write.table(sortKEGG,file=paste('0',i,'.',rownames(hub.exp)[i],'(GSE100927)','.xls',sep=''),sep="\t",quote=F,row.names=F)
  paths <- rownames(sortKEGG[c(1:5),])#选取你需要展示的通路ID
  
  #特定通路作图
  #trace(gseaplot2,edit=T)
  p = gseaplot2(KEGG,paths,base_size =10,color = c('#7B68EE','#CD3333','#20B2AA','#FF8C00','#FF6666'),rel_heights = c(1.5, 0.3, 0.5),title=rownames(hub.exp)[i])
  fn1 = paste0("0", i, ".", rownames(hub.exp)[i],'(GSE100927)', ".png")
  fn2 = paste0("0", i, ".", rownames(hub.exp)[i],'(GSE100927)', ".pdf")
  ggsave(fn1, p, width = 10, height = 6, units = "in", limitsize = 300)
  ggsave(fn2, p, width = 10, height = 6, units = "in", limitsize = 300)
  i<-i+1
}
