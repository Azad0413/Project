...
if (! dir.exists("./02_DEGs(RA)")){
  dir.create("./02_DEGs(RA)")
}
setwd("./02_DEGs(RA)")
## 读取数据
library(magrittr)
...
library(lance)
library(limma)
df = read.delim2("../00_rawdata/dat(GSE55457).xls", row.names = 1) %>% lc.tableToNum
df <- log2(df+1)
df.group = read.delim2("../00_rawdata/group(GSE55457).xls")
table(df.group$group)
df = df[df.group$sample]
df.group$group = factor(df.group$group, levels = c("control", "RA"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   RA = ifelse(df.group$group == "control", 0, 1))

contrast.mat = makeContrasts(contrasts="RA-control", levels=design.mat)
...
# DOWN  NOT   UP 
#  157    0  223 
write.table(DEG,file = "DEG_all(GSE55457).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE55457).xls",quote = F,sep = "\t",row.names = T)
## 火山图------
library(ggplot2)
library(ggthemes)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="#00CED1",RA="#F08080"))
