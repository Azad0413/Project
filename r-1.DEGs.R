...
setwd("./01_DEGs")
library(lance)
dat<-read.delim2('/data/nas1/luchunlin/project/YCZK-127//00_rawdata/dat.tcga.xls',row.names = 1)%>%lc.tableToNum()
...
dat_rep<-DEG[rownames(DEG)%in%
               c(head(rownames(subset(sig_diff,sig_diff$log2FoldChange>5.3)),10),
                 head(rownames(subset(sig_diff,sig_diff$log2FoldChange< -7.1)),10)),]
