...
if (! dir.exists("./12_checkpoint")){
  dir.create("./12_checkpoint")
}
setwd("./12_checkpoint")
...
scores2 <- scores
colnames(scores2)<-c('sample','socres','OS','OS.time','label')
dim(dat.tcga)
## 把检验点基因提取出来
...
violin_plot
ggsave(filename = 'checkpoint.pdf',violin_plot,w=13,h=6)
ggsave(filename = 'checkpoint.png',violin_plot,w=13,h=6)
