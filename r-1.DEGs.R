...
df.group = read.delim2("../00_rawdata/group.xls")
df = df[df.group$sample]
table(df.group$group)
df.group$group<-gsub(' ','_',df.group$group,fixed = T)
df.group$group = factor(df.group$group, levels = c("Low_BMD", "High_BMD"))
design.mat = cbind(Low_BMD = ifelse(df.group$group == "Low_BMD", 1, 0), 
                   High_BMD = ifelse(df.group$group == "Low_BMD", 0, 1))

contrast.mat = makeContrasts(contrasts="High_BMD-Low_BMD", levels=design.mat)
...
x[x>2]<-2
x[x<-2]<--2
table(group_rt$group)
ann_colors<-list(
  group = c(Low_BMD="#00CED1",High_BMD="#F08080"))
