...
if (! dir.exists("./03_exp(TCGA-match)")){
  dir.create("./03_exp(TCGA-match)")
}
setwd("./03_exp(TCGA-match)")
dat<-read.csv('../00_rawdata/dat.fpkm.xls',sep = '\t')
group <- read.delim2('../00_rawdata/group.match.xls')
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  labs(title="Expression(TCGA-match)", x="", y = "",size=20) +
  stat_pvalue_manual(stat.test,
                     y.position = c(0.27,12,8.5,0.2,4.5,5,10),
                     size = 3.2,
                     family = "Times",
                     label = "p",
                     #parse = T,
...
ggsave('01.expression(TCGA-match).pdf',exp_plot,width = 8,height = 6)
ggsave('01.expression(TCGA-match).png',exp_plot,width = 8,height = 6)
