...
if (! dir.exists("./12_relationship")){
  dir.create("./12_relationship")
}
setwd("./12_relationship")
library(GOSemSim)
library(reshape2)
...
write.table(fsim,file = 'relationship.xls',
            sep = '\t',
            quote = F)
ggsave('relationship.pdf',width = 6,height = 4)
ggsave('relationship.png',width = 6,height = 4)
