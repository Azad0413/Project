...
df<-read.delim2('/data/nas1/luchunlin/project/BJTC-321/01_DEGs/DEG_sig.xls',row.names = 1)
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  stat_pvalue_manual(stat.test,
                     y.position = c(7,7.5,7,15,7,10,14,11,12,8,13,13,6,11),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
...
exp_plot
ggsave('expression.pdf',exp_plot,width = 8,height = 8)
ggsave('expression.png',exp_plot,width = 8,height = 8)
