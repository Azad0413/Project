...
library(ggvenn)
mydata<-list('DEGs'=rownames(sig.diff),'lipid metabolism'=lipid$Geneset)
ggvenn(mydata,c('DEGs','lipid metabolism'),
...
library(enrichplot)
library(GOplot)
gene_transform <- bitr(DENRG$Geneset,
                       fromType = "SYMBOL",
                       toType = c("ENTREZID"),
                       OrgDb = "org.Hs.eg.db")
...
## 读取logFC文件
sigind<-sig.diff[rownames(sig.diff)%in%DENRG$Geneset,]
