...
library(tidyverse)
library(lance)
dat<-read.delim2('../01_DEGs/dat_final.xls',row.names = 1)%>%lc.tableToNum()
...
table(net$colors)  
# 0    1    2    3    4    5    6    7    8 
# 5527 2821  984  930  861  332  133  125  116 
## 04-4 层级聚类数展示各个模块
## 灰色的为**未分类**到模块的基因。
...
labeledHeatmap(Matrix = moduleTraitCor[-9,],
               xLabels = names(design_traits),
               yLabels = names(MEs[,-9]),
               ySymbols = names(MEs[,-9]),
               colorLabels = FALSE,
               colors = blueWhiteRed(50),
               textMatrix = textMatrix[-9,],
               setStdMargins = FALSE,
               cex.text = 0.7,
               zlim = c(-1,1),
...
par(mfrow = c(1,1),
    mai = c(1,1,1,0.5))
pdf('06.green_gene_cor.pdf',w=6,h=6)
...
dev.off()
png('06.green_gene_cor.png',w=600,h=600)
verboseScatterplot(abs(geneModuleMembership[moduleGenes, module_column]),
                   abs(geneTraitCor[moduleGenes, pheno_column]),
                   xlab = paste("Module Membership in", module, "module"),
                   ylab = paste("Gene significance for", pheno),
...
# par(mfrow = c(1,1),
#     mai = c(1,1,1,0.5))
# pdf('07.black_gene_cor.pdf',w=6,h=6)
# verboseScatterplot(abs(geneModuleMembership[moduleGenes, module_column]),
#                    abs(geneTraitCor[moduleGenes, pheno_column]),
#                    xlab = paste("Module Membership in", module, "module"),
#                    ylab = paste("Gene significance for", pheno),
...
#                    cex.main = 1.2, cex.lab = 1.2, cex.axis = 1.2, col = module)
# dev.off()
# png('07.black_gene_cor.png',w=600,h=600)
# verboseScatterplot(abs(geneModuleMembership[moduleGenes, module_column]),
#                    abs(geneTraitCor[moduleGenes, pheno_column]),
#                    xlab = paste("Module Membership in", module, "module"),
#                    ylab = paste("Gene significance for", pheno),
