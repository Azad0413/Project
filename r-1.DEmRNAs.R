...
if (! dir.exists("./01_DEmRNAs")){
  dir.create("./01_DEmRNAs")
}
setwd("./01_DEmRNAs")
library(lance)
## mRNA---------
dat<-read.delim2('../00_rawdata/dat.tcga(mRNA).xls',row.names = 1)%>%lc.tableToNum()
...
dev.off()
png('heatmap(mRNA).png',w=700,h=600)
pheatmap(mat=x,
         annotation_col = group_rt,
         color=bluered(100),
         #       annotation_row = annotation_raw,
