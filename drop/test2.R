...
if (! dir.exists("./01_DEG(validation2)")){
  dir.create("./01_DEGs(validation2)")
}
setwd("./01_DEGs(validation2)")
library(lance)
library(tidyverse)
library(magrittr)
library(stringr)
library(limma)
df = read.delim2("../00_rawdata/dat(GSE23561).xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2("../00_rawdata/group(GSE23561).xls")
...
# DOWN  NOT   UP 
# 597     0 14762 
write.table(DEG,file = "DEG_all(GSE23561).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig((GSE23561).xls",quote = F,sep = "\t",row.names = T)
...
test_expr <- read.delim2("../00_rawdata/dat(GSE23561).xls",row.names = 1)%>%lc.tableToNum()
test_group <- read.delim2('../00_rawdata/group(GSE23561).xls')
