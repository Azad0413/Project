...
if (! dir.exists("./09_validation(RA)")){
  dir.create("./09_validation(RA)")
}
setwd("./09_validation(RA)")
##单基因ROC
library(lance)
library(tidyverse)
hubgene <- read.delim2('../05_PPI/hubgene.xls')
dat<-read.delim2('../00_rawdata/dat(GSE12021).xls',row.names = 1)%>%lc.tableToNum()
group<-read.delim2("../00_rawdata/group(GSE12021).xls")
table(group$group)
group$group = factor(group$group, levels = c("RA", "control"))
...
library(pROC)
for (i in c(1:9)) {
  roc<-roc(group$group,hub_exp2[,i],levels=c("RA", "control"))
