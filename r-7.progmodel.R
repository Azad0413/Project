...
if (! dir.exists("./07_progmodel")){
  dir.create("./07_progmodel")
}
setwd("./07_progmodel")
...
train_phenotype$group<-ifelse(train_phenotype$group=='High',1,0)
...
colnames(train_phenotype2)<-c('id','Stage','Age','Scores','OS','OS.time','group')
train_phenotype2$Scores<-as.numeric(train_phenotype2$Scores)
class(train_phenotype2$Scores)
train_risk_clinical <- train_phenotype2
...
library(survival)
res.score = coxph(Surv(time = OS.time, event = OS) ~ Scores, data = train_risk_clinical) %>% summary
res.score = c(res.score$conf.int[-2], res.score$coefficients[5])
...
res.ref = c(1,1,1,NA)
res = rbind(res.score,res.age,res.ref,res.stage) %>% as.data.frame()
rownames(res)
res$Indicators = c("Aging score","Age","Stage1/2(Reference)",'Stage3','Stage4')
...
dev.off()
## 07-2 多因素Cox----------
res.mul = coxph(Surv(time = OS.time, event = OS) ~ Scores +Age+Stage, data = train_risk_clinical)%>% summary
res.mul = cbind(res.mul$conf.int[,-2], res.mul$coefficients[,5]) %>% as.data.frame()
rownames(res.mul)
res.mul = rbind(res.mul[c(1:2),], res.ref, res.mul[c(3:4),])
res.mul$Indicators = c("Aging Score","Age","Stage1/2(Reference)",'Stage3','Stage4')
...
coxm_9 <- cph(cox_data_prog,
              ...
              x=T,y=T,
              time.inc = 9*365)
cal_9 <-calibrate(coxm_9,u=9*365,cmethod='KM',m=100,B=100)
...
plot(cal_5,
     ...
     xlab='Nomogram-Predicted Probability of 5-9 year Survival Probability)',#便签
     ylab='Actual 5-9 year Survival Probability',#标签
     col="#00468b",#设置一个颜色
