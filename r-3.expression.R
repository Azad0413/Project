...
if (! dir.exists("./03_expression")){
  dir.create("./03_expression")
}
setwd("./03_expression")
...
exp_plot
ggsave(filename = '01.expression.pdf',exp_plot,w=5,h=5)
ggsave(filename = '01.expression.png',exp_plot,w=5,h=5)



dat<-read.csv('../00_rawdata/dat(GSE114868).xls',sep = '\t',row.names = 1)
group<-read.delim2('../00_rawdata/group(GSE114868).xls')
control.sample<-group$sample[which(group$group=='control')]
aml.sample<-group$sample[which(group$group=='AML')]
## UGCG-------
...
exp_plot <- ggplot(hub_exp2,aes(x = Symbol, y = expr, fill = Group)) +
  ...
  geom_boxplot(width=0.7,
               ...
               outlier.shape = NA)+ #绘制箱线图，此处width=0.1控制小提琴图中箱线图的宽窄
  scale_fill_manual(values= c("#4682B4","#CD3700"), name = "Group")+
  labs(title="Expression Level of UGCG in GSE114868", x="", y = "expression level",size=20) +
  stat_compare_means(data = hub_exp2,
                     mapping = aes(group = Group),
                     label ="p.signif",
                     method = 'wilcox.test') +
  ...
  theme(plot.title = element_text(hjust =0.5,colour="black",face="bold",size=15),
        ...
        panel.grid.minor = element_blank())+facet_wrap(~Symbol,scales = "free",nrow = 1) 
exp_plot
ggsave(filename = '01.expression(GSE114868).pdf',exp_plot,w=5,h=5)
ggsave(filename = '01.expression(GSE114868).png',exp_plot,w=5,h=5)


dat<-read.csv('../00_rawdata/dat(GSE796056).xls',sep = '\t',row.names = 1)
group<-read.delim2('../00_rawdata/group(GSE79605).xls')
control.sample<-group$sample[which(group$group=='control')]
aml.sample<-group$sample[which(group$group=='AML')]
## UGCG-------
...
exp_plot <- ggplot(hub_exp2,aes(x = Symbol, y = expr, fill = Group)) +
  ...
  geom_boxplot(width=0.7,
               ...
               outlier.shape = NA)+ #绘制箱线图，此处width=0.1控制小提琴图中箱线图的宽窄
  scale_fill_manual(values= c("#4682B4","#CD3700"), name = "Group")+
  labs(title="Expression Level of UGCG in GSE79605", x="", y = "expression level",size=20) +
  stat_compare_means(data = hub_exp2,
                     mapping = aes(group = Group),
                     label ="p.signif",
                     method = 't.test') +
  ...
  theme(plot.title = element_text(hjust =0.5,colour="black",face="bold",size=15),
        ...
        panel.grid.minor = element_blank())+facet_wrap(~Symbol,scales = "free",nrow = 1) 
exp_plot
ggsave(filename = '02.expression(GSE79605).pdf',exp_plot,w=5,h=5)
ggsave(filename = '02.expression(GSE79605).png',exp_plot,w=5,h=5)
