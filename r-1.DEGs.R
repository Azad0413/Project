...
library(limma)
library(readxl)
df = read.delim2("../00_rawdata/dat(GSE169568).xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2('../00_rawdata/group(GSE169568).xls')
df = df[df.group$sample]
table(df.group$group)
df.group$group = factor(df.group$group, levels = c("control", "UC"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   UC = ifelse(df.group$group == "control", 0, 1))
contrast.mat = makeContrasts(contrasts="UC-control", levels=design.mat)
...
# DOWN  NOT   UP 
# 438    0  338 
write.table(DEG,file = "DEG_all(GSE169568).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE169568).xls",quote = F,sep = "\t",row.names = T)
## 火山图------
library(ggplot2)
library(ggthemes)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="#20B2AA",UC="#F08080"))
