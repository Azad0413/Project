...
library(lance)
dat<-read.delim2('../10_External_va/dat.va.xls',row.names = 1)%>%lc.tableToNum()
group<-read.delim2('../10_External_va/risk_out.xls')
high.sample<-group$id[which(group$risk_out==0)]
low.sample<-group$id[which(group$risk_out==1)]

hubgene<-read.csv('../08_Lasso/lasso_genes.csv',header = F)
