...
dat<-read.delim2("../00_rawdata/dat(GSE23561).xls", row.names = 1)%>% lc.tableToNum
group = read.delim2("../00_rawdata/group(GSE23561).xls")
table(group$group)
...
exp_plot
ggsave('01.expression.pdf',exp_plot,width = 7,height = 5)
ggsave('01.expression.png',exp_plot,width = 8,height = 6)
...
ggsave(filename = "02.model.verify.png", width = 5, height = 5)
ggsave(filename = "02.model.verify.pdf", width = 5, height = 5)
...
for (i in c(1:8)) {
  ...
  png(paste0("0",i+2, ".", colnames(hub_exp2)[i],".png"),width = 300,height = 300)
  plot(roc,
       print.auc=T,
       ...
       legacy.axes=T)
  dev.off()
  pdf(paste0("0",i+2, ".", colnames(hub_exp2)[i],".pdf"),width = 4,height = 4)
  plot(roc,
       print.auc=T,
       print.auc.x=0.4,print.auc.y=0.5,
       #auc.polygon=T,
...
# # 绘制PCA得分图
# library(ggplot2)
# p.pca1 <- ggplot(data = df1,aes(x = PC1,y = PC2,color = group$group))+
...
# p.pca1
# ggsave(p.pca1,filename = '11.pca.pdf',w=6,h=5)
# ggsave(p.pca1,filename = '11.pca.png',w=6,h=5)