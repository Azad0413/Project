...
HT.sample<-group$sample[which(group$group=='HT')]
...
}
{
  ...
  for (i in 1:nrow(tiics_result)){
    pvalue[i, 1] = p.value = wilcox.test(tiics_result[i, HT.sample],
                                         tiics_result[i, control.sample])$p.value
    log2FoldChange[i, 1] = mean(tiics_result[i, HT.sample]) - 
  ...
  HT <- signif(apply(tiics_result[rownames(rTable), HT.sample], 
                       1,
                       mean), 4)
  control <- signif(apply(tiics_result[rownames(rTable), control.sample], 
                      1, 
                      mean), 4)
  rTable <- data.frame(HT, 
                       control,
...
box_dat$group<-ifelse(box_dat$sample%in%HT.sample,'HT','control')
...
violin_dat$group <- ifelse(violin_dat$sample%in%HT.sample,'HT','control')
#library(rstatix)
...
library(cowplot)
plot_grid(p1,p2,p3,p4,p5,p6,p7,p8,p9,p10,labels = c("A", "B", "C", "D",'E','F','G','H','I','J'), ncol = 2)  
ggsave("combine.pdf",w=16,h=19)
ggsave("combine.png",w=16,h=19)
