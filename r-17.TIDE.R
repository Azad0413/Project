...
library(ggstatsplot)
tide_cor2 <- ggscatterstats(data = tide_plot_dat,
                           y = riskScore,
                           x = Dysfunction,
                           centrality.para = "mean",
                           margins = "both",
                           ...
                           yfill = "#0073C2FF",
                           type = "pearson",
                           xlab = "Dysfunction Prediction Score",
                           marginal.type = "histogram",
                           title = "Relationship between Dysfunction Prediction Score and riskScore"
)+theme(axis.title.x = element_text(size = 15),
        axis.title.y = element_text(size = 15),
        plot.title = element_text(size = 12))
tide_cor2
...
tide_box2 <- ggboxplot(tide_plot_dat,
                      x = "risk_group",
                      y = "Dysfunction",
                      fill = "risk_group",
                      palette =c("#A73030FF", "#0073C2FF")) +
  stat_compare_means(label.y = 3.2) +
  theme_bw() +
  labs(title = "", x = "", y = "Dysfunction Prediction Score") +
  theme(plot.title = element_text(hjust =0.5,colour="black",face="bold",size=12),
        axis.text.x=element_text(angle = 45,hjust = 1,colour="black",face="bold",size=12), 
        axis.text.y=element_text(hjust=0.5,colour="black",face="bold",size=12),
        axis.title.y=element_text(size=12,face="bold"),
...
# ggsave(filename = "02.tide_riskscore_boxplot.pdf", height = 5, width = 4,tide_box)
library(patchwork)
tide_plot2 <- tide_cor2+tide_box2+
  plot_layout(ncol = 2,heights = c(8,1), widths = c(3,1))
tide_plot2

ggsave(filename = '02.Dysfunction_plot.pdf',tide_plot2,w=9,h=7)
ggsave(filename = '02.Dysfunction_plot.png',tide_plot2,w=9,h=7)


##2 Exclusion-----

library(ggstatsplot)
tide_cor3 <- ggscatterstats(data = tide_plot_dat,
                            y = riskScore,
                            x = Exclusion,
                            centrality.para = "mean",
                            margins = "both",
                            ...
                            yfill = "#0073C2FF",
                            type = "pearson",
                            xlab = "Exclusion Prediction Score",
                            marginal.type = "histogram",
                            title = "Relationship between Exclusion Prediction Score and riskScore"
)+theme(axis.title.x = element_text(size = 15),
        axis.title.y = element_text(size = 15),
        plot.title = element_text(size = 12))
tide_cor3
...
tide_box3 <- ggboxplot(tide_plot_dat,
                       x = "risk_group",
                       y = "Exclusion",
                       fill = "risk_group",
                       palette =c("#A73030FF", "#0073C2FF")) +
  stat_compare_means(label.y = 3.2) +
  theme_bw() +
  labs(title = "", x = "", y = "Exclusion Prediction Score") +
  theme(plot.title = element_text(hjust =0.5,colour="black",face="bold",size=12),
        axis.text.x=element_text(angle = 45,hjust = 1,colour="black",face="bold",size=12), 
        axis.text.y=element_text(hjust=0.5,colour="black",face="bold",size=12),
        axis.title.y=element_text(size=12,face="bold"),
...
# ggsave(filename = "02.tide_riskscore_boxplot.pdf", height = 5, width = 4,tide_box)
library(patchwork)
tide_plot3 <- tide_cor3+tide_box3+
  plot_layout(ncol = 2,heights = c(8,1), widths = c(3,1))
tide_plot3

ggsave(filename = '03.Exclusion_plot.pdf',tide_plot3,w=9,h=7)
ggsave(filename = '03.Exclusion_plot.png',tide_plot3,w=9,h=7)
