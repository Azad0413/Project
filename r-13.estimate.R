...
if (! dir.exists("./13_ESTIMATE")){
  dir.create("./13_ESTIMATE")
}
setwd("./13_ESTIMATE")

dat<-read.csv('../00_rawdata/dat.fpkm.xls',sep = '\t',row.names = 1)
colnames(dat)<-gsub('.','-',colnames(dat),fixed = T)
group<-read.delim2('../07_risk/risk.xls')%>%dplyr::select(c('id','riskScore','risk'))
...
ggsave(filename = '01.estimate.all.pdf',p5,w=12,h=4)
ggsave(filename = '01.estimate.all.png',p5,w=12,h=4)
