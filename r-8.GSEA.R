...
gogmt<-read.gmt("/data/nas1/luchunlin/pipeline/GSVA/c5.go.bp.v7.4.symbols.gmt") #读gmt文件
# 每个hub基因均分析一遍 ------------------------------------------------------------
##分组
for (i in c(1:8)){
  train<-t(hub.exp)
  ...
  GO<-GSEA(geneList,TERM2GENE = gogmt,pvalueCutoff = 0.05) #GSEA分析
  sortGO<-data.frame(GO)
  sortGO<-sortGO[order(sortGO$p.adjust, decreasing = F),]#按照enrichment score从高到低排序
  write.table(sortGO,file=paste('0',i,'.',rownames(hub.exp)[i],'(GO)','.xls',sep=''),sep="\t",quote=F,row.names=F)
  paths <- rownames(sortGO[c(1:5),])#选取你需要展示的通路ID
  
  #特定通路作图
  #trace(gseaplot2,edit=T)
  p = gseaplot2(GO,paths,base_size =10,color = c('#7B68EE','#CD3333','#20B2AA','#FF8C00','#FF6666'),rel_heights = c(1.5, 0.3, 0.5),title=rownames(hub.exp)[i])
  fn1 = paste0("0", i, ".", rownames(hub.exp)[i],'(GO)', ".png")
  fn2 = paste0("0", i, ".", rownames(hub.exp)[i],'(GO)',".pdf")
  ggsave(fn1, p, width = 8, height = 5, units = "in", limitsize = 300)
  ggsave(fn2, p, width = 8, height = 5, units = "in", limitsize = 300)
  i<-i+1
}
