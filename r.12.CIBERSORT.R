...
setwd("/data/nas1/luchunlin/project/LZZK-512/12_CIBERSORT/")
{
...
box_plot
ggsave('boxplot.pdf',box_plot,w=10,h=7)
ggsave('boxplot.png',box_plot,w=10,h=7)
## 13-2 小提琴图--------
...
violin_plot
ggsave('violin.pdf',violin_plot,w=10,h=9)
ggsave('violin.png',violin_plot,w=10,h=9)



##3 相关性----------
hubgene<-read.delim2('/data/nas1/luchunlin/project/LZZK-512/04_Lasso/lasso_genes.csv',header = F,col.names = 'hubgene')
hubgene<-hubgene[-8,]%>%as.data.frame()
colnames(hubgene)<-'hubgene'
train<-dat.tcga[hubgene$hubgene,]
...
while(cnt<12){
  ...
  p10
  ggsave(paste(cnt, ".", rownames(train)[cnt],'pdf',sep='.'),w=8,h=6)
  ggsave(paste(cnt, ".", rownames(train)[cnt],'png',sep='.'),w=8,h=6)
