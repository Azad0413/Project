...
colData<-data.frame(sample=rownames(dat),
                    group=ifelse(dat$CIRBP>median(dat$CIRBP),'High','Low'))
...
dat_rep<-DEG[rownames(DEG)%in%
               c(head(rownames(subset(sig_diff,sig_diff$log2FoldChange>1.48)),10),
                 head(rownames(subset(sig_diff,sig_diff$log2FoldChange< -1.19)),10)),]
