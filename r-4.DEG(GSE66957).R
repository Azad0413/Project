...
if (! dir.exists("./04_DEGs(GSE66957)")){
  dir.create("./04_DEGs(GSE66957)")
}
setwd("./04_DEGs(GSE66957)")
## 读取数据
library(magrittr)
library(stringr)
library(lance)
library(limma)
df = read.delim2("../00_rawdata/dat(GSE66957).xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2("../00_rawdata/group(GSE66957).xls")
...
# DOWN  NOT   UP 
# 6489    0 9782 
write.table(DEG,file = "DEG_all(GSE66957).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE66957).xls",quote = F,sep = "\t",row.names = T)
...
volcano_plot<- ggplot(data = DEG, 
  ...
  labs(x = "log (Fold Change)",
       y = "-log10 (adj.P.Val)")
volcano_plot
ggsave('01.volcano(GSE66957).png', volcano_plot,width = 8, height = 6)
ggsave('01.volcano(GSE66957).pdf', volcano_plot,width = 8, height = 6)
### 热图--------
library(pheatmap)
library(ggplot2)
library(RColorBrewer)
