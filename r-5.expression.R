...
df<-read.delim2('/data/nas1/luchunlin/project/BJTC-345/01_DEGs/DEG_sig.xls',row.names = 1)
## 
hubgene <- data.frame(symbol=c('GALC','SPTLC2','SMPD2','DEGS2','DEGS1','SMPD3'))
...
## 样本分组
hub_exp2$Group<-ifelse(hub_exp2$sample%in%control.sample,'control','NPC')
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  labs(title="Expression of hubgene", x="", y = "",size=20) +
  stat_pvalue_manual(stat.test,
                     y.position = c(12,8,11,10,7,9),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
  ...
  guides(fill='none')+facet_wrap(~Symbol,scales = "free",nrow = 2) 
exp_plot
ggsave('expression.pdf',exp_plot,width = 6,height = 5)
ggsave('expression.png',exp_plot,width = 6,height = 5)
