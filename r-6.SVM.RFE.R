...
library(mlbench)
library(caret)
filtedat <- read.delim2('../00_rawdata/dat(GSE65682).xls',row.names = 1)%>%lc.tableToNum()
dat_group <- read.delim2('../00_rawdata/group(GSE65682).xls')%>%column_to_rownames(var = 'sample')
