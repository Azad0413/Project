...
library(ggcorrplot)
library(corrplot)
de.dat <- checkpoint_dat[DE.checkpoint$gene,]%>%select(-'gene')
hubgene <- read.delim2('../05_Lasso/lasso_genes.csv',header = F)
cor.dat <- dat.tcga[hubgene$V1,]
cor.dat <- log2(cor.dat+1)
library(Hmisc)
nc<-t(rbind(de.dat,cor.dat))
m=rcorr(nc,type = 'spearman')$r[1:nrow(de.dat),(ncol(nc)-length(rownames(cor.dat))+1):ncol(nc)]
m<-t(m)

p=rcorr(nc,type = 'spearman')$P[1:nrow(de.dat),(ncol(nc)-length(rownames(cor.dat))+1):ncol(nc)]
p<-t(p)
...
colnames(textMatrix)<-colnames(cor)
rownames(textMatrix)<-rownames(cor)
write.table(textMatrix,file = 'correlaion.xls',sep = '\t',row.names = T,quote = F)



library(WGCNA)
pdf(file = '02.correlation.pdf',w=10,h=5)
par(mar = c(9, 9, 3, 1));
labeledHeatmap(Matrix = cor, 
               ...
               cex.text = 0.8, 
               zlim = c(-1,1),
               main = paste("modelgenes-DEcheckpoint correlation"))
dev.off()
png(file = '02.correlation.png',w=850,h=350)
par(mar = c(9, 9, 3, 1));
labeledHeatmap(Matrix = cor, 
               xLabels = colnames(cor), 
               yLabels = rownames(cor), 
