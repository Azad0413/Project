...
library(lance)
library(limma)
df = read.delim2("/data/nas1/luchunlin/project/BJTC-317/00_rawdata/dat.final.xls", row.names = 1) %>% lc.tableToNum 
df<-log2(df+1)
df.group = read.delim2("/data/nas1/luchunlin/project/BJTC-317/00_rawdata/group.xls")
df = df[df.group$sample]
df.group$group = factor(df.group$group, levels = c("control", "PD"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   PD = ifelse(df.group$group == "control", 0, 1))

contrast.mat = makeContrasts(contrasts="PD-control", levels=design.mat)
...
dat_rep<-DEG[rownames(DEG)%in%
               c(head(rownames(subset(sig_diff,sig_diff$logFC>1.46)),10),
                 head(rownames(subset(sig_diff,sig_diff$logFC< -2.21)),10)),]
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="#00CED1",PD="#F08080"))
