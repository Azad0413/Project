...
if (! dir.exists("./03_uncox(TIIC)")){
  dir.create("./03_uncox(TIIC)")
}
setwd("./03_uncox(TIIC)")
## 单因素cox--------
library(tidyverse)
library(lance)
#lasso.gene<-read.csv('../02_Lasso(TIIC)/lasso_genes.csv',header = F)
...
lasso_geneids
## [1] "CHPF"       "PYGB"       "GLT8D2"     "B3GALNT2"   "ALG9"       "GALNT15"    "ST6GALNAC3" "B3GALT2"    "B4GALNT2"   "ST8SIA6"    "GYS2"       "GALNTL6" 
write(lasso_geneids, "lasso_genes.csv")
write.csv(x_all,file = "Lasso_x.csv",quote = F)
write.csv(y_all,file = "Lasso_y.csv",quote = F)
#  多因素cox回归--------
setwd("/data/nas1/luchunlin/project/JNZK-214-8/")
if (! dir.exists("./04_Mulcox(TIIC)")){
  dir.create("./04_Mulcox(TIIC)")
}
setwd("./04_Mulcox(TIIC)")
library(survival)
library(survminer)
gene_list<-lasso_geneids
#gene_list <- rownames(res_results_0.2)
...
# 05 风险模型的构建与验证------
setwd("/data/nas1/luchunlin/project/JNZK-214-8/")
if (! dir.exists("./05_risk(TIIC)")){
  dir.create("./05_risk(TIIC)")
}
setwd("./05_risk(TIIC)")
...
pd<-pData(a)
table(pd$`biological source:ch1`)
pd<-pd[which(pd$`biological source:ch1`=='primary bladder cancer'),]
write.table(dat.va,file = 'dat.va.xls',sep = '\t',quote = F,row.names = T)
survival.va<-data.frame(sample=pd$geo_accession,OS=pd$`overall survival:ch1`,OS.time=pd$`survival month:ch1`)
...
survival.va$OS<-ifelse(survival.va$OS=='death',1,0)
...
unmap<-dat.va[c('GALNT17','GALNTL2','GALNTL1'),]
rownames(unmap)<-c('GALNTL6','GALNT15','GALNT16')
