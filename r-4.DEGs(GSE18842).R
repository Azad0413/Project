...
if (! dir.exists("./04_DEGs(GSE18842)")){
  dir.create("./04_DEGs(GSE18842)")
}
setwd("./04_DEGs(GSE18842)")
...
library(limma)
library(readxl)
df = read.delim2("../00_rawdata/dat(GSE18842).xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2('../00_rawdata/group(GSE18842).xls')
...
# DOWN  NOT   UP 
#402    0  280 
write.table(DEG,file = "DEG_all(GSE18842).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE18842).xls",quote = F,sep = "\t",row.names = T)
## 火山图------
library(ggplot2)
library(ggthemes)
