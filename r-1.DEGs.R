...
library(lance)
library(limma)
df = read.delim2("/data/nas1/luchunlin/project/BJTC-302/00_rawdata/dat.xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2("/data/nas1/luchunlin/project/BJTC-302/00_rawdata/group.xls")
df = df[df.group$sample]
df.group$group = factor(df.group$group, levels = c("control", "Burn"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   Burn = ifelse(df.group$group == "control", 0, 1))

contrast.mat = makeContrasts(contrasts="Burn-control", levels=design.mat)
...
dat_rep<-DEG[rownames(DEG)%in%
               c(head(rownames(subset(sig_diff,sig_diff$logFC>6.2)),10),
                 head(rownames(subset(sig_diff,sig_diff$logFC< -3.15)),10)),]
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="#00CED1",Burn="#F08080"))
