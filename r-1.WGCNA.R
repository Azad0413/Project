...
# 1142 2050 1968 1736  927  840  674  382  346  290  206  179  174  142  125   65   62   55   43   42   37   33 
## 04-4 层级聚类数展示各个模块
## 灰色的为**未分类**到模块的基因。
...
labeledHeatmap(Matrix = moduleTraitCor[-22,],
               xLabels = names(design_traits),
               yLabels = names(MEs[,-22]),
               ySymbols = names(MEs[,-22]),
               colorLabels = FALSE,
               colors = blueWhiteRed(50),
               textMatrix = textMatrix[-22,],
               setStdMargins = FALSE,
               cex.text = 0.7,
               zlim = c(-1,1),
