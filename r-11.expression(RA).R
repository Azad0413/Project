...
if (! dir.exists("./11_expression(RA)")){
  dir.create("./11_expression(RA)")
}
setwd("./11_expression(RA)")
dat<-read.delim2("../00_rawdata/dat(GSE55457).xls", row.names = 1)%>% lc.tableToNum
group <- read.delim2('../00_rawdata/group(GSE55457).xls')
table(group$group)
hubgene <- read.delim2('../05_PPI/hubgene.xls')

diff <- read.delim2('../02_DEGs(RA)/DEG_sig(GSE55457).xls',row.names = 1)
...
## 样本分组
hub_exp2$Group<-ifelse(hub_exp2$sample%in%control.sample,'control','RA')
...
hub_exp2$Group <- factor(hub_exp2$Group,levels = c('RA','control'))
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  stat_pvalue_manual(stat.test,
                     y.position = c(15,16,16,15,15,16,16,16,12),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
...
## 训练集
dat.va<-read.delim2('../00_rawdata/dat(GSE12021).xls',row.names = 1)%>%lc.tableToNum()
dat.va <- na.omit(dat.va)
#colnames(dat.va) <- gsub('X','',colnames(dat.va),fixed = T)
group.va <- read.delim2('../00_rawdata/group(GSE12021).xls')
