...
library(tidyverse)
## 读取从xena下载的数据
tcga.expr<-read_tsv(file = 'TCGA-BLCA.htseq_counts.tsv.gz')
...
#pcg<-read.delim2('/data/nas1/luchunlin/pipeline/PCG/PCG.xls')
#dat.final<-dat.final[rownames(dat.final)%in%pcg$gene_name,]
write.table(dat.final,file = 'dat.tcga.xls',sep = '\t',quote = F,row.names = T)
##fpkm
expr_fpkm<-read_tsv(file = 'TCGA-BLCA.htseq_fpkm.tsv.gz')%>%as.data.frame()%>%column_to_rownames(var = 'Ensembl_ID')
## xena下载的数据经过了log2+1转化，需要将其还原
