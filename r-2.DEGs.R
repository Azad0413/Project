...
library(limma)
library(readxl)
df = read.delim2("../00_rawdata/dat(GSE10846).xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2('../01_subtype/cluster.xls')
df = df[df.group$sample]
colnames(df.group) <- c('sample','group')
table(df.group$group)
df.group$group <- ifelse(df.group$group=='cluster 1','cluster1','cluster2')
df.group$group = factor(df.group$group, levels = c("cluster2", "cluster1"))
design.mat = cbind(cluster2 = ifelse(df.group$group == "cluster2", 1, 0), 
                   cluster1 = ifelse(df.group$group == "cluster2", 0, 1))
contrast.mat = makeContrasts(contrasts="cluster1-cluster2", levels=design.mat)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(cluster2="#20B2AA",cluster1="#F08080"))
