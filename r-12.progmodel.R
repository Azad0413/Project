...
if (! dir.exists("./12_progmodel")){
  dir.create("./12_progmodel")
}
setwd("./12_progmodel")

clinical<-read_tsv(file ='/data/nas1/luchunlin/TCGA_phenotype/TCGA-OV.GDC_phenotype.tsv')
...
colnames(train_phenotype2)<-c('id','Stage','Age','OS','OS.time')
...
rownames(res)
res$Indicators = c("riskScore","Age","Stage1/2(Reference)",'Stage3','Stage4')
...
res.mul$Indicators = c("riskScore","Age","Stage1/2(Reference)",'Stage3','Stage4')
