...
library(readr)
library(tidyverse)
survival<-read_tsv(file = 'TCGA-CESC.survival.tsv')

dat.tcga<-read.delim2("原始数据.txt", header = T,row.names = 1)%>% lc.tableToNum
colnames(dat.tcga)<-gsub('.','-',colnames(dat.tcga),fixed = T)
colnames(dat.tcga) <- substr(colnames(dat.tcga),1,16)
dat.tcga <- dat.tcga[,-c(1:3)]

hubgene<-read_xlsx('34个基因信息.xlsx')
#risk <- read.delim2('risk.xls',header = T)

survival<-survival[survival$sample%in%colnames(dat.tcga),]
survival <- survival[!duplicated(survival$'_PATIENT'),]
survival<-survival[,-3]
...
train_dat<-train_dat[,colnames(train_dat)%in%hubgene$Gene]
...
pdf(file = "01.univariate_cox_forest.pdf", height = 3, width = 9, onefile = F)
forestplot(labeltext=tabletext, 
           ...
           xticks = c(0,0.5,1,1.5,2,2.5,3), #横坐标刻度
           lwd.xaxis=2,            #X轴线宽
...
dev.off()
png(filename = "01.univariate_cox_forest.png", height = 250, width = 600)
forestplot(labeltext=tabletext, 
           graph.pos=4,  #为Pvalue箱线图所在的位置
           is.summary = c(TRUE, TRUE,rep(FALSE, 70)),
           col=fpColors(box="red", lines="royalblue", zero = "gray50"),
...
library(survival)
library(survminer)
gene_list <- c('PTPN6','LATS2','GCH1','CMKLR1')
cox_data <- as.formula(paste0('Surv(OS.time, OS)~',
                              paste(gene_list,
                                    sep = '',
                                    collapse = '+')))
exp_hubgene<-read.table('risk.xls',header=T,row.names=1)%>%lc.tableToNum()
train_data <- exp_hubgene[,c(1:6)]
colnames(train_data) <- c("OS.time","OS","PTPN6","LATS2","GCH1","CMKLR1")
...
cox_formula <- as.formula(paste("Surv(OS.time, OS)~",
                                paste(rownames(cox_table)[cox_table[,3]>0.04],
                                      collapse = "+")))
cox_more_2 <- coxph(cox_formula, data = train_data)
# 计算共线性
cox_correlation <- cor(train_data[, rownames(cox_table)[cox_table[,3]>0.04]],
                       method = "pearson")
library(GGally)
cox_corr <- ggpairs(train_data[, rownames(cox_table)[cox_table[,3]>0.04]],
                    axisLabels = "show") +
  theme_bw() +
...
nrow(tabletext) + 1
library(forestplot)
pdf(file = "02.multivariate_cox_forest.pdf", height = 3, width = 9, onefile = F)
forestplot(labeltext=tabletext, 
           graph.pos=4,  #为Pvalue箱线图所在的位置
           ...
           grid = T) # 垂直于x轴的网格线，对应每个刻度
dev.off()
png(file = "02.multivariate_cox_forest.png", height = 200, width = 900)
forestplot(labeltext=tabletext, 
           graph.pos=4,  #为Pvalue箱线图所在的位置
           is.summary = c(TRUE, TRUE,rep(FALSE, 57)),
           col=fpColors(box="red", lines="royalblue", zero = "gray50"),
...
class(cox_more_2$coefficients)

cox_more_2$coefficients <-as.numeric(c('-0.06239','0.121284','0.145344','-0.04982')) 
names(cox_more_2$coefficients) <- c('PTPN6','LATS2','GCH1','CMKLR1')
cox_more_2$coefficients
...
# 0   1 
# 42 43 
risk <- read.table('/data/nas1/tancj/Project/BJTC-202_repair/risk.xls',header=T)
colnames(risk) <- c("id","OS.time","OS","PTPN6","LATS2","GCH1","CMKLR1","riskScore","risk")

riskScore <- risk$riskScore
table(risk$risk)
risk$risk <- ifelse(risk$risk=='high',0,1)
...
##  risk 0.0086 Gehan-Breslow p = 0.0086
...
library(gplots)
mul.dat<-dat.tcga[rownames(res_results_0.05_2),]
...
survival.va <- read_xlsx('survival.va.xlsx')
survival.va<-data.frame(sample=pd$geo_accession,OS=pd$`alive-or-dead:ch1`,OS.time=pd$`days-followup:ch1`)
...
survival.va$OS<-ifelse(survival.va$OS=='D',1,0)
...
# 1 risk_out 0.0287 Gehan-Breslow p = 0.029
...
library(RColorBrewer)
library(gplots)
mul.va<-dat.va[rownames(res_results_0.05_2),]
