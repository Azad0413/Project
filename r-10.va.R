...
if (! dir.exists("./10_validation")){
  dir.create("./10_validation")
}
setwd("./10_validation")
...
group$group<-ifelse(group$group=='Control','control','Burn')
table(group$group)
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  stat_pvalue_manual(stat.test,
                     y.position = c(13,15,14,15,17,16,15,14),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
...
hubgene<-hubgene[c(1:6),]%>%as.data.frame()
colnames(hubgene)<-'hubgene'
write.table(hubgene,file = 'hubgene.final.xls',sep = '\t',row.names = F,quote = F)
