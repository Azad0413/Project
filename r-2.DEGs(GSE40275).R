...
if (! dir.exists("./02_DEGs(GSE40275)")){
  dir.create("./02_DEGs(GSE40275)")
}
setwd("./02_DEGs(GSE40275)")
...
library(limma)
library(readxl)
df = read.delim2("../00_rawdata/dat(GSE40275).xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2('../00_rawdata/group(GSE40275).xls')
...
# DOWN  NOT   UP 
#401    0  517 
write.table(DEG,file = "DEG_all(GSE40275).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE40275).xls",quote = F,sep = "\t",row.names = T)
## 火山图------
library(ggplot2)
library(ggthemes)
