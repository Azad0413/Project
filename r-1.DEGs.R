...
library(DESeq2)
# colData <- data.frame(sample=colnames(dat),group=c(rep('Normal',5),rep('Tumor',167)))
colData<-read.delim2('../00_rawdata/group.merge.xls')
table(colData$group)
colData$group <- ifelse(colData$group=='normal','Normal','Tumor')
...
volcano_plot<- ggplot(data = DEG, 
  ...
  scale_y_continuous(trans = "log1p",
                     breaks = c(0,5,50, 100,200,400,500)) +
...
group_rt$sample <- gsub('-','.',group_rt$sample,fixed = T)
rt<-normalized_counts[,group_rt$sample]
