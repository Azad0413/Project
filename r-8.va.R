...
table(group$group)
group<-group[which(group$group=='Healthy control'|group$group=='Asthmatic at baseline'),]
group$group<-ifelse(group$group=='Healthy control','control','asthma')
table(group$group)
write.table(group,file = 'group.va.xls',sep = '\t',row.names = F,quote = F)
write.table(dat,file = 'dat_va.xls',sep = '\t',row.names = T,quote = F)
hubgene<-read.delim2('/data/nas1/luchunlin/project/BJTC-321/06_roc/hub.final.xls')
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  stat_pvalue_manual(stat.test,
                     y.position = c(6.6,9,12,8.5,12,14,15,13,11),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
...
exp_plot
ggsave('expression.pdf',exp_plot,width = 7,height = 7)
ggsave('expression.png',exp_plot,width = 7,height = 7)
hubgene
hub.final<-hubgene[-c(3,4,6),]%>%as.data.frame()
colnames(hub.final)<-'hubgene'
write.table(hub.final,file = 'hub_final.xls',sep = '\t',row.names = F,quote = F)
