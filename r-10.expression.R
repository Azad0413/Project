...
if (! dir.exists("./10_expression")){
  dir.create("./10_expression")
}
setwd("./10_expression")
## 训练集
...
exp_plot
ggsave('01.expression(train).pdf',exp_plot,width = 8,height = 5)
ggsave('01.expression(train).png',exp_plot,width = 8,height = 5)


## 训练集
dat.va<-read.delim2('../00_rawdata/dat.va.xls',row.names = 1)%>%lc.tableToNum()
dat.va <- na.omit(dat.va)
group.va <- read.delim2('../00_rawdata/group.va.xls')
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  facet_wrap(~Symbol,scales = "free",nrow = 2) 
exp_plot
ggsave('02.expression(validation).pdf',exp_plot,width = 8,height = 5)
ggsave('02.expression(validation).png',exp_plot,width = 8,height = 5)
