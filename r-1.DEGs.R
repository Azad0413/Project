...
if (! dir.exists("./01_DEGs(GSE79056)")){
  dir.create("./01_DEGs(GSE79056)")
}
setwd("./01_DEGs(GSE79056)")
...
library(limma)
library(readxl)
df = read.csv('../00_rawdata/dat(GSE796056).xls',row.names = 1,sep = '\t')
df.group = read.delim2('../00_rawdata/group(GSE79605).xls')
df = df[df.group$sample]
table(df.group$group)
df.group$group = factor(df.group$group, levels = c("control", "AML"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   AML = ifelse(df.group$group == "control", 0, 1))
contrast.mat = makeContrasts(contrasts="AML-control", levels=design.mat)
...
dat_rep <- rbind(intreast,dat_rep)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="#20B2AA",AML="#F08080"))
