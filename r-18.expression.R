...
if (! dir.exists("./18_expression")){
  dir.create("./18_expression")
}
setwd("./18_expression")
...
## 样本分组
hub_exp2$Group<-ifelse(hub_exp2$sample%in%control.sample,'control','CHOL')
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  stat_pvalue_manual(stat.test,
                     y.position = c(11,14,13,10,12,12),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
...
exp_plot
ggsave('01.expression(train).pdf',exp_plot,width = 7,height = 5)
ggsave('01.expression(train).png',exp_plot,width = 7,height = 5)


## 训练集
dat.va<-read.delim2('../09_External_va/dat.va.xls',row.names = 1)%>%lc.tableToNum()
dat.va <- na.omit(dat.va)
colnames(dat.va) <- gsub('X','',colnames(dat.va),fixed = T)
group.va <- read.delim2('../09_External_va/group.va.xls')
table(group.va$group)

control.sample <- group.va$sample[which(group.va$group=='control')]

#1 validation-----
hub_exp<-dat.va[hubgene$V1,]
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  facet_wrap(~Symbol,scales = "free",nrow = 2) 
exp_plot
ggsave('02.expression(validation).pdf',exp_plot,width = 7,height = 5)
ggsave('02.expression(validation).png',exp_plot,width = 7,height = 5)
