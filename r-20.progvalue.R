...
if (! dir.exists("./20_progvalue")){
  dir.create("./20_progvalue")
}
setwd("./20_progvalue")
library(tidyverse)
survival<-read.delim2("../05_univariate_cox/survival.xls")
phenotype<-read_tsv(file = '/data/nas1/luchunlin/TCGA_phenotype/TCGA-STAD.GDC_phenotype.tsv.gz')
...
res = rbind(res.risk,res.age,res.gender,res.ref,res.stage,res.ref,res.T_stage,res.ref,res.N_stage,res.M_stage,res.grade) %>% as.data.frame()
rownames(res)
res$Indicators = c("riskScore","Age",'Gender','Stage1(Reference)',"Stage2",'Stage3','Stage4',"T1/2(Reference)",'T3','T4',"N0(Reference)",'N1','N2','N3','M1 vs.M0','Grade3 vs.Grade1/2')
...
forestplot(labeltext=tabletext, 
           ...
           is.summary = c(TRUE,FALSE,FALSE,FALSE,TRUE,FALSE,FALSE,FALSE,TRUE,FALSE,FALSE,TRUE,rep(FALSE, 7)),
...
png(file = '01.uncoxforest.png',height = 500, width = 900)
forestplot(labeltext=tabletext, 
           graph.pos=4,  #为Pvalue箱线图所在的位置
           is.summary = c(TRUE,FALSE,FALSE,FALSE,TRUE,FALSE,FALSE,FALSE,TRUE,FALSE,FALSE,TRUE,rep(FALSE, 7)),
           col=fpColors(box="red", lines="royalblue", zero = "gray50"),
...
dev.off()
## 07-2 多因素Cox----------
res.mul = coxph(Surv(time = OS.time, event = OS) ~ riskScore +Age+Stage+T.stage+N.stage+M.stage, data = train_risk_clinical)%>% summary
res.mul = cbind(res.mul$conf.int[,-2], res.mul$coefficients[,5]) %>% as.data.frame()
rownames(res.mul)

res.mul = rbind(res.mul[c(1:2),], res.ref, res.mul[c(3:5),],res.ref,res.mul[c(6:7),],res.ref,res.mul[c(8:11),])
res.mul$Indicators = c("riskScore","Age",'Stage1(Reference)',"Stage2",'Stage3','Stage4',"T1/2(Reference)",'T3','T4',"N0(Reference)",'N1','N2','N3','M1 vs.M0')
...
forestplot(labeltext=tabletext, 
           graph.pos=4,  #为Pvalue箱线图所在的位置
           is.summary = c(TRUE,FALSE,FALSE,TRUE,FALSE,FALSE,FALSE,TRUE,FALSE,FALSE,TRUE,FALSE,FALSE,FALSE,FALSE,FALSE),
           col=fpColors(box="red", lines="royalblue", zero = "gray50"),
...
coxm_2 <- cph(cox_data_prog,
              ...
              x=T,y=T,
              time.inc = 2*365)
cal_2 <-calibrate(coxm_2,u=2*365,cmethod='KM',m=100,B=200)
