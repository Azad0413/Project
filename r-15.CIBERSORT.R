...
cibersort_plot
ggsave(filename = '02.cibersort.plot.pdf',cibersort_plot,w=6,h=6)
ggsave(filename = '02.cibersort.plot.png',cibersort_plot,w=6,h=6)
...
m=rcorr(nc)$r[1:nrow(diff_score),(ncol(nc)-length(hubgene$V1)+1):ncol(nc)]
m<-t(m)
p=rcorr(nc)$P[1:nrow(diff_score),(ncol(nc)-length(hubgene$V1)+1):ncol(nc)]
p<-t(p)
library(dplyr)
library(dplyr)
...
png('03.heatmap.png',w=400,h=300)
pheatmap(m,
         display_numbers =tmp,
         angle_col =45,
         color = colorRampPalette(c("#92b7d1", "white", "#d71e22"))(100),
