...
if (! dir.exists("./02_DElncRNAs")){
  dir.create("./02_DElncRNAs")
}
setwd("./02_DElncRNAs")
library(lance)
## lncRNA---------
dat<-read.delim2('../00_rawdata/dat.tcga(lncRNA).xls',row.names = 1)%>%lc.tableToNum()
colnames(dat)<-gsub('.','-',colnames(dat),fixed = T)
dat<-round(dat,digits = 0)
library(DESeq2)
colData<-data.frame(sample=colnames(dat),
                    group=c(rep('Normal',113),rep('Tumor',1082)))
...
write.table(DEG_write, file = "DEG_all(lncRNA).xls",
            quote = F,
            sep = "\t",
            row.names = F)
sig_diff_write <- cbind(GeneSymbol=rownames(sig_diff), sig_diff)
write.table(sig_diff_write, file = "DEG_sig(lncRNA).xls",
            quote = F,
            sep = "\t",
            row.names = F)
### 火山图---------
...
volcano_plot<- ggplot(data = DEG, 
  ...
  labs(x = "log (Fold Change)",
       y = "-log10 (adj.P.Val)")
volcano_plot
ggsave('volcano(lncRNA).png', volcano_plot,width = 8, height = 6)
ggsave('volcano(lncRNA).pdf', volcano_plot,width = 8, height = 6)
### 热图--------
library(pheatmap)
library(ggplot2)
library(RColorBrewer)
...
pdf('heatmap(lncRNA).pdf',w=8,h=6)
pheatmap(mat=x,
...
dev.off()
png('heatmap(lncRNA).png',w=700,h=600)
pheatmap(mat=x,
         annotation_col = group_rt,
         color=bluered(100),
         #       annotation_row = annotation_raw,
