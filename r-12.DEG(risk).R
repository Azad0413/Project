...
if (! dir.exists("./12_DEGs(risk)")){
  dir.create("./12_DEGs(risk)")
}
setwd("./12_DEGs(risk)")
library(lance)
library(tidyverse)
...
library(limma)
library(readxl)
df = read.delim2("../00_rawdata/dat.xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2('../08_risk/risk.xls')%>%select(c('id','riskScore'))
df.group$riskScore <- ifelse(df.group$riskScore>median(df.group$riskScore),'High_risk','Low_risk')
colnames(df.group) <- c('sample','group')
df = df[,df.group$sample]
table(df.group$group)
df.group$group = factor(df.group$group, levels = c("Low_risk", "High_risk"))
design.mat = cbind(Low_risk = ifelse(df.group$group == "Low_risk", 1, 0), 
                   High_risk = ifelse(df.group$group == "Low_risk", 0, 1))
contrast.mat = makeContrasts(contrasts="High_risk-Low_risk", levels=design.mat)
...
# DOWN  NOT   UP 
#66    0   39 
write.table(DEG,file = "DEG_all(risk).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(risk).xls",quote = F,sep = "\t",row.names = T)
## 火山图------
library(ggplot2)
library(ggthemes)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(Low_risk="#00CED1",High_risk="#F08080"))
