...
library(limma)
library(readxl)
df = read.delim2("../00_rawdata/dat(GSE65682).xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2('../00_rawdata/group(GSE65682).xls')
df = df[df.group$sample]
table(df.group$group)
df.group$group = factor(df.group$group, levels = c("control", "Sepsis"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   Sepsis = ifelse(df.group$group == "control", 0, 1))
contrast.mat = makeContrasts(contrasts="Sepsis-control", levels=design.mat)
...
# DOWN  NOT   UP 
#1000    0  489 
write.table(DEG,file = "DEG_all(GSE65682).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE65682).xls",quote = F,sep = "\t",row.names = T)
## 火山图------
library(ggplot2)
library(ggthemes)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="#20B2AA",Sepsis="#F08080"))
