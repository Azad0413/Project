...
library(tidyverse)
## A1 vs A2------
DEM <- read.csv('t_test(A1vsA2).csv',row.names = 1)
...
dim(sig_diff)
# 18
write.table(sig_diff,file = 'DEM_sig(A1vs.A2).xls',sep = '\t',row.names = T,quote = F)
...
volcano_plot<- ggplot(data = DEM, 
  ...
  labs(x = "log (Fold Change)",
       y = "-log10 (p.value)",title = 'A1vs.A2')
volcano_plot
ggsave('01.volcano(A1vs.A2).png', volcano_plot,width = 6, height = 4)
ggsave('01.volcano(A1vs.A2).pdf', volcano_plot,width = 6, height = 4)


## A2 vs A3------
DEM <- read.csv('t_test(A2vsA3).csv',row.names = 1)
colnames(DEM)
logFC_cutoff <- 1
DEM$change = as.factor(
...
dim(sig_diff)
# 37
write.table(sig_diff,file = 'DEM_sig(A2vs.A3).xls',sep = '\t',row.names = T,quote = F)
# write.table(DEM,file = 'DEM_all(A2vs.A3).xls',sep = '\t',row.names = T,quote = F)
colnames(sig_diff)
dat_rep<-DEM[rownames(DEM)%in%
               rownames(rbind(head(sig_diff[order(sig_diff$log2.FC.,decreasing = T),],10),
                              head(sig_diff[order(sig_diff$log2.FC.,decreasing = F),],10))),]
volcano_plot<- ggplot(data = DEM, 
  ...
  #   point.padding = unit(0.8, "lines"), segment.color = "black", show.legend = FALSE )+
  labs(x = "log (Fold Change)",
       y = "-log10 (p.value)",title = 'A2vs.A3')
volcano_plot
ggsave('02.volcano(A2vs.A3).png', volcano_plot,width = 6, height = 4)
ggsave('02.volcano(A2vs.A3).pdf', volcano_plot,width = 6, height = 4)
