...
## ESCA包括食管鳞癌（ESCC）和食管腺癌（ESA），只要ESCC
...
## 筛选癌症组织，去掉癌旁组织。01-09为肿瘤，10-19为正常对照
mete=data.frame(colnames(dat_fpkm))  # 取第一行样本id
for (i in 1:length(mete[,1])) {
  num=as.numeric(as.character(substring(mete[i,1],14,15)))
  if(num %in% seq(1,9)){mete[i,2]="T"}
  if(num %in% seq(10,29)){mete[i,2]="N"}
...
mete$group=as.factor(mete$group)
mete=subset(mete,mete$group=="T")
dat_fpkm_tumor<-dat_fpkm[,which(colnames(dat_fpkm)%in%mete$id)]
escc_fpkm<-dat_fpkm_tumor[,colnames(dat_fpkm_tumor)%in%clinical_ESCC$sample]
dim(escc_fpkm)
## 81
write.table(escc_fpkm,file = 'escc.xls',sep = '\t',row.names = T,quote = F)
# # fpkm转TPM
