...
if (! dir.exists("./02_WGCNA(POAG)")){
  dir.create("./02_WGCNA(POAG)")
}
setwd("./02_WGCNA(POAG)")
...
# 2164 5239 1574  522  469  455  408  347  304  236  158  127   81   78   72   58   53 
## 04-4 层级聚类数展示各个模块
## 灰色的为**未分类**到模块的基因。
...
dev.off()
pdf(file = "03.cluster_dendrogram.pdf", height = 7, width = 10)
plotDendroAndColors(net$dendrograms[[1]], moduleColors[net$blockGenes[[1]]],
                    "Module colors",
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05)
...
sizeGrWindow(6,6)
pdf('04.Eigengene_dendrogram_and_heatmap.pdf',w=7,h=7)
...
dev.off()
png('04.Eigengene_dendrogram_and_heatmap.png',w=700,h=700)
plotEigengeneNetworks(MEs_col, 
                      setLabels = "Eigengene dendrogram and heatmap", 
                      marDendro = c(1,3,4,4),
                      marHeatmap = c(3,3,0,2), 
...
## 1655
write.table(modGenes,file = 'modGene(POAG).xls',sep = '\t',quote = F,row.names = F)
