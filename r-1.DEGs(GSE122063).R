...
if (! dir.exists("./01_DEGs(GSE122063)")){
  dir.create("./01_DEGs(GSE122063)")
}
setwd("./01_DEGs(GSE122063)")
## 读取数据
library(magrittr)
library(stringr)
library(lance)
library(limma)
df = read.delim2("../00_rawdata/dat(GSE122063).xls", row.names = 1) %>% lc.tableToNum
#df <- log2(df)
df.group = read.delim2("../00_rawdata/group(GSE122063).xls")
df = df[df.group$sample]
table(df.group$group)
df.group$group = factor(df.group$group, levels = c("control", "VaD"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   VaD = ifelse(df.group$group == "control", 0, 1))

contrast.mat = makeContrasts(contrasts="VaD-control", levels=design.mat)
...
# DOWN  NOT   UP 
# 1348    0  967 
write.table(DEG,file = "DEG_all(GSE122063).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE122063).xls",quote = F,sep = "\t",row.names = T)
...
volcano_plot<- ggplot(data = DEG, 
  ...
  labs(x = "log (Fold Change)",
       y = "-log10 (adj.P.Val)")
volcano_plot
ggsave('01.volcano(GSE122063).png', volcano_plot,width = 8, height = 6)
ggsave('01.volcano(GSE122063).pdf', volcano_plot,width = 8, height = 6)
### 热图--------
library(pheatmap)
library(ggplot2)
library(RColorBrewer)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="#00CED1",VaD="#F08080"))
