...
library(enrichplot)
library(limma)
kegggmt<-read.gmt("c2.cp.kegg.v7.5.1.symbols.gmt") #读gmt文件
dat<-read.delim2("/data/nas1/luchunlin/project/BJTC-317/00_rawdata/dat.final.xls", row.names = 1)  %>% lc.tableToNum%>%log2()
hubgene <- read.delim2('/data/nas1/luchunlin/project/BJTC-317/06_Logistic/hubgene.xls')
hub.exp<-dat[hubgene$sybmol,]
