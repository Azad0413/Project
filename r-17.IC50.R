...
library(ggplot2)
set.seed(12345)
model_geneids<-read.delim2('../07_Lasso/lasso_genes.csv',header = F)
...
drugs_hub_boxplot
ggsave(filename = "01.drugs.plot(down).pdf", height = 9, width = 13)
ggsave(filename = "01.drugs.plot(down).png", height = 9, width = 13)
...
drugs_hub_boxplot <- facet(drugs_hub_boxplot1,
  ...
  theme(panel.grid = element_blank(),
        ...
        text = element_text(size = 13, face = "bold"))
drugs_hub_boxplot
ggsave(filename = "02.drugs.plot(up).pdf", height = 11, width = 14)
ggsave(filename = "02.drugs.plot(up).png", height = 11, width = 14)
