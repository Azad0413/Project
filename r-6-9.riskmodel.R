...
## 9
## [1]"PDIA4"    "GNS"      "OSMR"     "MXRA8"    "PDLIM4"   "STC1"     "C9orf64"  "SLC16A13" "GZMB"    
...
risk_dis <- ggplot(risk, aes(x=reorder(id, riskScore), 
  ...
  scale_x_discrete(breaks = risk[order(risk$riskScore),]$id[c(1,50,100,150,200)],
...
expr.va <- read_xlsx('CGGA.mRNAseq_325.RSEM-genes.20200506.xlsx')%>%column_to_rownames(var = 'Gene_Name')%>%lc.tableToNum()
survival.va <- read_xlsx('survival.va2.xlsx')
dat.va <- expr.va[,survival.va$CGGA_ID]
FPKM2TPM <- function(fpkm){
  exp(log(fpkm) - log(sum(fpkm)) + log(1e6))
}

dat.va <- apply(dat.va,2,FPKM2TPM)
write.table(dat.va,file = 'dat.va.xls',sep = '\t',quote = F,row.names = T)
survival.va<-data.frame(sample=survival.va$CGGA_ID,OS=survival.va$`Censor (alive=0; dead=1)`,
                        OS.time=survival.va$OS)
...
#test_data<-log2(test_data+0.00001)
...
risk_dis_out <- ggplot(risk_out, aes(x=reorder(id, riskScore_out), 
  ...
  geom_point() +
  scale_color_manual(values = c("#A73030FF", "#0073C2FF")) + 
  scale_x_discrete(breaks = risk_out[order(risk_out$riskScore_out),]$id[c(1,25,50,75,100,125,150)],
