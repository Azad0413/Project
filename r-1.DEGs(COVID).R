...
if (! dir.exists("./01_DEGs(COVID)")){
  dir.create("./01_DEGs(COVID)")
}
setwd("./01_DEGs(COVID)")
library(tidyverse)
library(lance)
...
library(DESeq2)
colData<-read.delim2('../00_rawdata/group(GSE171110).xls')
table(colData$group)
colData$group<-factor(colData$group,levels = c('control','Covid_19'))
...
dds<-DESeq(dds)
## 提取差异结果
res =results(dds, contrast = c("group","Covid_19","control"))
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="lightblue",Covid_19="darkorange"),
