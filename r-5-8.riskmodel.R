...
library(readr)
library(tidyverse)
survival<-read_tsv(file = '/data/nas1/luchunlin/TCGA_survival/TCGA-STAD.survival.tsv')
survival<-survival[,-3]
...
forestplot(labeltext=tabletext, 
           ...
           xticks = c(0,0.2,0.4,0.6,0.8,1,1.2), #横坐标刻度
           lwd.xaxis=2,            #X轴线宽
...
dat.va <- read.csv('../00_rawdata/dat.va(GSE62254).xls',sep = '\t',row.names = 1)
survival.va <- read.csv('../00_rawdata/survival.va(GSE62254).xls',sep = '\t')
