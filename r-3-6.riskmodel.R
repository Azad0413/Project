...
group<-read.delim2("/data/nas1/luchunlin/project/YCZK-127/01_DEGs/group.xls")
ov.sample<-group$sample[which(group$group=='Tumor')]
##1104
#dat.tcga<-log2(dat.tcga+1)
DEIRG<-read.delim2('/data/nas1/luchunlin/project/YCZK-127/02_DEIRG/DEIRG.xls')
survival<-survival[survival$sample%in%colnames(dat.tcga),]
write.table(survival,file = 'survival.xls',sep = '\t',quote = F,row.names = F)
train_dat<-dat.tcga[,ov.sample]
...
## 5
## [1] "KRT19"  "PSME2"  "HMGB3"  "MRPL13" "SHCBP1"
...
survival.va<-data.frame(sample=pd$geo_accession,OS=pd$`event_death:ch1`,OS.time=pd$`follow_up_duration (years):ch1`)
survival.va$OS.time<-as.numeric(survival.va$OS.time)*365
