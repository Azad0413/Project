...
group = read.delim2("/data/nas1/luchunlin/project/BJTC-357/00_rawdata/group(geo).xls")
control.sample<-group$sample[which(group$group=='control')]
df<-read.delim2('/data/nas1/luchunlin/project/BJTC-357/01_DEGs/DEG_sig.xls',row.names = 1)
## 
lassogene<-read.csv(file = '../04_Lasso/lasso_genes.csv',header = F)
lassogene$V1<-gsub('_','-',lassogene$V1,fixed = T)
hubgene <- data.frame(symbol=lassogene$V1)
...
## 样本分组
hub_exp2$Group<-ifelse(hub_exp2$sample%in%control.sample,'control','ESCC')
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  stat_pvalue_manual(stat.test,
                     y.position = c(14,16,18,14,12,16,15),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
