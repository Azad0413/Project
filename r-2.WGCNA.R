...
dataExprVar <- dataExpr[which(m.mad >
                                max(quantile(m.mad, probs=seq(0, 1, 0.0001))[2],0.01)),]
## 转换为样品在行，基因在列的矩阵
...
pdf('01.sample_tree.pdf',w=11,h=6)
plot(sampleTree, main = "Sample clustering to detect outliers", sub="", xlab="")
abline(h =45, col = "red")
dev.off()
png('01.sample_tree.png',w=1000,h=500)
plot(sampleTree, main = "Sample clustering to detect outliers", sub="", xlab="")
abline(h =45, col = "red")
dev.off()
...
# 6510 2755 1803 1071  429  169  114  112  100   96   52   45   42   41   34   34   26 
## 04-4 层级聚类数展示各个模块
## 灰色的为**未分类**到模块的基因。
...
module3='red'
probes3=colnames(dataExpr)
inModule3=(moduleColors==module3)
modProbes3=probes3[inModule3]
