...
load('/data/nas1/luchunlin/project/JNZK-255-2/riskmodel.RData')
...
colnames(clinic)
res.cut<-surv_cutpoint(clinic,time = 'futime',event = 'fustat',variables = 'riskscore')
res.cut$cutpoint
clinic$Risk<-ifelse(clinic$riskscore>res.cut$cutpoint[,1],'High_risk','Low_risk')
...
## 堆叠图
High_risk<-subset(box_dat,Risk=='High_risk')
Low_risk<-subset(box_dat,Risk=='Low_risk')
## Low_risk
data1 <- data.frame(group=Low_risk$Risk,response=Low_risk$response)
data1<-as.data.frame(sort(table(data1$response)))
colnames(data1)<-c('Response','Low_risk')
## High_risk
data2<-data.frame(group=High_risk$Risk,response=High_risk$response)
data2<-as.data.frame(sort(table(data2$response)))
colnames(data2)<-c('Response','High_risk')
data<-merge(data1,data2,by='Response')
library(reshape2)
library(plyr)
library(RColorBrewer)
mydata <- melt(data,id.vars="Response",variable.name="Risk",value.name="Quantity")
p<-ggplot(mydata,aes(x=Risk,y=Quantity,fill=Response))+
...
p
ggsave(p,filename = '01.boxplot.pdf',w=5,h=5)
ggsave(p,filename = '01.boxplot.png',w=5,h=5)
## 卡方检验
mydata<-box_dat
...
violin_plot <- ggplot(box_dat, aes(x=response, 
  ...
  stat_compare_means(data = box_dat,
                     ...
                     # label ="p.signif",
                     method = 'wilcox.test',
                     paired = F,label.y = 3.5,label.x = 1.4) +
...
ggsave(width=6,height=6,'03.violin(CR_PD).pdf')
ggsave(width=6,height=6,'03.violin(CR_PD).png')

# # KM ----------------------------------------------------------------------
kmfit<-survfit(Surv(futime,fustat) ~ Risk, data =  clinic)
pdf('04.KM.pdf',w=7,h=6,onefile=F,family='Times')
