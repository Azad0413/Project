...
setwd("/data/nas1/luchunlin/project/BJTC-302/11_CIBERSORT/")
{
  tiics_result <- cibersort_result
  pvalue = padj = log2FoldChange <- matrix(0, nrow(tiics_result), 1)
  for (i in 1:nrow(tiics_result)){
    pvalue[i, 1] = p.value = wilcox.test(tiics_result[i, Burn.sample],
                                         tiics_result[i, control.sample])$p.value
    log2FoldChange[i, 1] = mean(tiics_result[i, Burn.sample]) - 
  ...
  Burn <- signif(apply(tiics_result[rownames(rTable), Burn.sample], 
                       1,
                       mean), 4)
  control <- signif(apply(tiics_result[rownames(rTable), control.sample], 
                      1, 
                      mean), 4)
  rTable <- data.frame(Burn, 
                       control,
...
box_dat$group<-ifelse(box_dat$sample%in%Burn.sample,'Burn','control')
...
violin_dat$group <- ifelse(violin_dat$sample%in%Burn.sample,'Burn','control')
#library(rstatix)
...
violin_plot
ggsave('violin.pdf',violin_plot,w=12,h=8)
ggsave('violin.png',violin_plot,w=12,h=8)
