...
if (! dir.exists("./05_DEGs(GSE40595)")){
  dir.create("./05_DEGs(GSE40595)")
}
setwd("./05_DEGs(GSE40595)")
## 读取数据
library(magrittr)
library(stringr)
library(lance)
library(limma)
df = read.delim2("../00_rawdata/dat(GSE40595).xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2("../00_rawdata/group(GSE40595).xls")
...
# DOWN  NOT   UP 
# 6061    0 5107 
write.table(DEG,file = "DEG_all(GSE40595).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE40595).xls",quote = F,sep = "\t",row.names = T)
...
volcano_plot<- ggplot(data = DEG, 
  ...
  labs(x = "log (Fold Change)",
       y = "-log10 (adj.P.Val)")
volcano_plot
ggsave('01.volcano(GSE40595).png', volcano_plot,width = 8, height = 6)
ggsave('01.volcano(GSE40595).pdf', volcano_plot,width = 8, height = 6)
### 热图--------
library(pheatmap)
library(ggplot2)
library(RColorBrewer)
