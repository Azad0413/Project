...
df.group = read_xlsx('../00_rawdata/group.xlsx')
df = df[df.group$sample]
table(df.group$group)
df.group$group = factor(df.group$group, levels = c("control", "CHOL"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   CHOL = ifelse(df.group$group == "control", 0, 1))
contrast.mat = makeContrasts(contrasts="CHOL-control", levels=design.mat)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="#00CED1",CHOL="#F08080"))
