...
library(lance)
library(limma)
df = read.delim2("/data/nas1/luchunlin/project/BJTC-308/00_rawdata/dat2.xls", row.names = 1) %>% lc.tableToNum
mRNA<-read.table(file = 'mRNA.txt')
df<-df[mRNA$gene_name,]
df.group = read.delim2("/data/nas1/luchunlin/project/BJTC-308/00_rawdata/group2.xls")
...
dat_rep<-DEG[rownames(DEG)%in%
               c(head(rownames(subset(sig_diff,sig_diff$logFC>4.28)),10),
                 head(rownames(subset(sig_diff,sig_diff$logFC< -4.36)),10)),]
