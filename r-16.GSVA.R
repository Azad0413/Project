...
group<-read.delim2('../07_risk/risk.xls')%>%dplyr::select(c('id','riskScore'))
...
KEGG_ref <- getGmt("/data/nas1/luchunlin/pipeline/GSVA/h.all.v2022.1.Hs.symbols.gmt")
...
p
ggsave(filename = '01.GSVA.pdf',p,w=12,h=12)
ggsave(filename = '01.GSVA.png',p,w=12,h=12)
