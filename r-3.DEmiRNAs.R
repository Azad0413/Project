...
if (! dir.exists("./03_DEmiRNAs")){
  dir.create("./03_DEmiRNAs")
}
setwd("./03_DEmiRNAs")
library(lance)
## miRNA---------
dat<-read.delim2('../00_rawdata/dat.tcga(miRNA).xls',row.names = 1)%>%lc.tableToNum()
...
write.table(DEG_write, file = "DEG_all(miRNA).xls",
            quote = F,
            sep = "\t",
            row.names = F)
sig_diff_write <- cbind(GeneSymbol=rownames(sig_diff), sig_diff)
write.table(sig_diff_write, file = "DEG_sig(miRNA).xls",
            quote = F,
            sep = "\t",
            row.names = F)
### 火山图---------
...
volcano_plot<- ggplot(data = DEG, 
  ...
  labs(x = "log (Fold Change)",
       y = "-log10 (adj.P.Val)")
volcano_plot
ggsave('volcano(miRNA).png', volcano_plot,width = 8, height = 6)
ggsave('volcano(miRNA).pdf', volcano_plot,width = 8, height = 6)
### 热图--------
library(pheatmap)
library(ggplot2)
library(RColorBrewer)
...
dev.off()
png('heatmap(miRNA).png',w=700,h=600)
pheatmap(mat=x,
         annotation_col = group_rt,
         color=bluered(100),
         #       annotation_row = annotation_raw,
