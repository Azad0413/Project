...
if (! dir.exists("./03_DEGs(GSE18520)")){
  dir.create("./03_DEGs(GSE18520)")
}
setwd("./03_DEGs(GSE18520)")
## 读取数据
library(magrittr)
library(stringr)
library(lance)
library(limma)
df = read.delim2("../00_rawdata/dat(GSE18520).xls", row.names = 1) %>% lc.tableToNum
df <- log2(df)
df.group = read.delim2("../00_rawdata/group(GSE18520).xls")
df = df[df.group$sample]
df.group$group = factor(df.group$group, levels = c("control", "OV"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   OV = ifelse(df.group$group == "control", 0, 1))

contrast.mat = makeContrasts(contrasts="OV-control", levels=design.mat)
...
# DOWN  NOT   UP 
#2862    0 8110 
write.table(DEG,file = "DEG_all(GSE18520).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE18520).xls",quote = F,sep = "\t",row.names = T)
...
volcano_plot<- ggplot(data = DEG, 
  ...
  labs(x = "log (Fold Change)",
       y = "-log10 (adj.P.Val)")
volcano_plot
ggsave('01.volcano(GSE18520).png', volcano_plot,width = 8, height = 6)
ggsave('01.volcano(GSE18520).pdf', volcano_plot,width = 8, height = 6)
### 热图--------
library(pheatmap)
library(ggplot2)
library(RColorBrewer)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="#00CED1",OV="#F08080"))
