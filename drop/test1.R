...
if (! dir.exists("./01_DEG(validation)")){
  dir.create("./01_DEGs(validation)")
}
setwd("./01_DEGs(validation)")
library(lance)
library(tidyverse)
library(magrittr)
library(stringr)
library(limma)
df = read.delim2("../00_rawdata/dat(GSE42148).xls", row.names = 1) %>% lc.tableToNum
df.group = read.delim2("../00_rawdata/group(GSE42148).xls")
...
# DOWN  NOT   UP 
# 277    0  391 
write.table(DEG,file = "DEG_all(GSE42148).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE42148).xls",quote = F,sep = "\t",row.names = T)
...
test_expr <- read.delim2("../00_rawdata/dat(GSE42148).xls",row.names = 1)%>%lc.tableToNum()
test_group <- read.delim2('../00_rawdata/group(GSE42148).xls')
