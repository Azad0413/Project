...
setwd("./01_DEGs")
library(lance)
dat<-read.delim2('/data/nas1/luchunlin/project/BJTC-308/00_rawdata/dat.tcga.xls',row.names = 1)%>%lc.tableToNum()
...
colData<-data.frame(sample=colnames(dat),
                    group=c(rep('Normal',11),rep('Tumor',162)))
...
dat_rep<-DEG[rownames(DEG)%in%
               c(head(rownames(subset(sig_diff,sig_diff$log2FoldChange>7)),10),
                 head(rownames(subset(sig_diff,sig_diff$log2FoldChange< -6.45)),10)),]
