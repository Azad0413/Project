...
if (! dir.exists("./03_DEG(subtype)")){
  dir.create("./03_DEG(subtype)")
}
setwd("./03_DEG(subtype)")
library(tidyverse)
library(lance)
...
library(DESeq2)
colData<-read.delim2('../02_cluster/cluster.xls')
colnames(colData) <- c('sample','group')
dat<-dat[,colData$sample]
table(colData$group)
colData$group <- ifelse(colData$group=='cluster 1','Cluster1','Cluster2')
