...
if (! dir.exists("./05_randomforest")){
  dir.create("./05_randomforest")
}
setwd("./05_randomforest")

library(magrittr)
library(ggplot2)
hub_gene <- read.delim2('../02_intersect/DECFRGs.xls')
train.dat<-read.delim2("../00_rawdata/dat(GSE169568).xls", row.names = 1)  %>% lc.tableToNum
...
write.table(result,file = 'rf.importance.xls',sep = '\t',row.names = T,quote = F)

top <- data.frame(symbol=rownames(result))
top <- top[-c(4,5,7,10),]%>%as.data.frame()
colnames(top) <- 'symbol'
write.table(top,file = 'rf.gene.xls',sep = '\t',row.names = F,quote = F)
