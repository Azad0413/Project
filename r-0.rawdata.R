...
group$group<-ifelse(group$group=='retina','control','RB')
group<-group[order(group$group),]
dat<-dat[,group$sample]
write.table(dat,file = 'dat(GSE97508).xls',sep = '\t',row.names = T,quote = F)
write.table(group,file = 'group(GSE97508).xls',sep = '\t',row.names = F,quote = F)
