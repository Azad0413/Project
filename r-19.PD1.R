...
load('/data/nas1/luchunlin/project/HZ0301-3/riskmodel.RData')
lasso_geneids<-as.character(lasso_geneids)
exp<-log2(eset+1)
...
cnt<-1
while (cnt < 349) {
  clinic$riskscore[cnt]<-sum(coef.min*subset(rt2,select=lasso_geneids)[cnt,])
  cnt = cnt + 1
}
cnt<-1
while (cnt < 349) {
...
clinic$Risk<-ifelse(clinic$risk==1,'Low_risk','High_risk')
...
riskscore <- as.numeric(clinic$riskscore)
gene <- as.numeric(pd1)
df1 <- cbind(gene,riskscore)%>%as.data.frame()
p1 <- ggplot(df1,aes(riskscore,gene))+
  xlab('riskscore')+ylab('PD-1')+
  geom_point()+geom_smooth(method = 'lm',formula = y~x)+
  theme_bw()+
  stat_cor(method = 'spearman',aes(x=riskscore,y=gene))
p1  
...
ggsave(filename = '01.PD1-risk-correlation.pdf',p2,w=4,h=4)
ggsave(filename = '01.PD1-risk-correlation.png',p2,w=4,h=4)
##B 不同分组的km------
pd1.group <- data.frame(sample=colnames(pd1),PD1=t(pd1)[,1])
pd1.group$PD1 <- ifelse(pd1.group$PD1>median(pd1.group$PD1),'High_PD1','Low_PD1')
table(pd1.group$PD1)
group.all <- merge(clinic,pd1.group,by.x='ID',by.y = 'sample')
table(group.all$Risk)
group.all$group <- paste0(group.all$Risk,',',group.all$PD1)


write.table(group.all,'clinic.txt',sep='\t',quote=F)
colnames(group.all)
kmfit<-survfit(Surv(futime, fustat) ~ group, data =  group.all)
table(group.all$group)
surv_pvalue(kmfit,method = 'FH_p=1_q=1')
# p = 0.033
cluster_survival_median <- ggsurvplot(kmfit,
                                      pval = TRUE, 
                                      conf.int = F,
                                      legend.labs=c("High_risk,High_PD1","High_risk,Low_PD1","Low_risk,High_PD1","Low_risk,Low_PD1"),
                                      legend.title="group",
                                      title="KM cruve",
                                      font.main = c(15,"bold"),
                                      risk.table = TRUE, 
                                      ...
                                      surv.median.line = "hv", 
                                      ggtheme = theme_bw(), 
                                      palette = c("#A73030FF", "#0073C2FF",'purple','orange'))
...
ggsave(width=6,height=6,'04.violin.pdf')
ggsave(width=6,height=6,'04.violin.png')
...
box_plot
ggsave(filename = '03.boxplot.pdf',w=5,h=5)
ggsave(filename = '03.boxplot.png',w=5,h=5)

mydata<-box_dat
ka<-xtabs(~mydata$response+mydata$Risk,data = mydata)
chisq.test(ka)
# p-value = 0.1685
...
violin_plot <- ggplot(box_dat, aes(x=response, 
  ...
  labs(title="", x="", y = "LRScore",size=20) +
  stat_compare_means(data = box_dat,
                     mapping = aes(group = response),
                     # label ="p.signif",
                     method = 'wilcox.test',
                     paired = F,label.y = 0,label.x = 1.4) +
...
violin_plot
ggsave(filename = '02.violin.pdf',w=5,h=5)
ggsave(filename = '02.violin.png',w=5,h=5)

# # KM ----------------------------------------------------------------------
# kmfit<-survfit(Surv(futime,fustat) ~ Risk, data =  clinic)
# pdf('03.KM.pdf',w=7,h=6,onefile=F,family='Times')
...
# dev.off()
# 
# png('03.KM.png',w=7,h=6,family='Times',units='in',res=600 )
# s_surv<-ggsurvplot(kmfit,
#                    pval = TRUE, conf.int = F,legend.labs=c("High","Low" ),legend.title="Risk", title="IMVigor210",
#                    risk.table = TRUE,
#                    risk.table.col = "strata",
...
# 
# lines(year_2$FP, year_2$TP, type="l",col="blue",xlim=c(0,1), ylim=c(0,1),lwd=2)
...
# dev.off()
# 
# png('05.ROC.png',w=9,h=8.5,units='in',res=600)
# par(pin = c(4,4), mar = c(5,6,6,1),family="serif")      ##par(family="serif")  :all
# # pin: 以英寸表示的图形尺寸（宽和高）
# # mai: 以数值向量表示的边界大小，顺序为“下、左、上、右”，单位为英寸
# # mar: 以数值向量表示的边界大小，顺序为“下、左、上、右”，单位为英分。默认（5,4,4,2）+ 0.1
