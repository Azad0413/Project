...
# 3283 2380 2075 1954 1841 1487 1417 1016  839  695  652  555  408  272  243  163   81   72   66   63 
## 04-4 层级聚类数展示各个模块
## 灰色的为**未分类**到模块的基因。
...
## 04-6 关联表型数据------
tiics_result <- read.delim2('../01_CIBERSORT/cibersort.txt',header = T,row.names = 1)
colnames(tiics_result) <- gsub('.','-',colnames(tiics_result),fixed = T)
DEcell <- read.delim2('../02_CIBERSORT(GEO)/DEcell.xls')
diff_tiics <- tiics_result[DEcell$ImmuneCell,]
trait<-t(diff_tiics)
### 模块与表型数据关联
if (corType=="pearson") {
...
purple_hub <- subset(c,c$MM > 0.5 & c$GS > 0.2)
##490
sizeGrWindow(7, 7)
par(mfrow = c(1,1),
    mai = c(1,1,1,0.5))
# 与性状高度相关的基因，也是与性状相关的模型的关键基因
pdf('05.purple_cor.pdf',w=7,h=6)
...
dev.off()
png('05.purple_cor.png',w=500,h=400)
verboseScatterplot(abs(geneModuleMembership[moduleGenes,module_column]),
                   abs(geneTraitCor[moduleGenes, 1]),
                   xlab = paste("Module Membership in", module, "module"),
                   ylab = "Gene significance for DEimmune Cell",
...
c<-as.data.frame(cbind(MM,GS))
rownames(c)=modGenes$modgene
lightgreen_hub <- subset(c,c$MM > 0.5 & c$GS > 0.2)
##3
sizeGrWindow(7, 7)
par(mfrow = c(1,1),
    mai = c(1,1,1,0.5))
# 与性状高度相关的基因，也是与性状相关的模型的关键基因
pdf('06.lightgreen_cor.pdf',w=7,h=6)
verboseScatterplot(abs(geneModuleMembership[moduleGenes,module_column]),
                   abs(geneTraitCor[moduleGenes, 1]),
                   xlab = paste("Module Membership in", module, "module"),
                   ylab = "Gene significance for DEimmune Cell",
...
abline(h=0.2,v=0.5,col="lightgreen",lwd=1.5)
dev.off()
png('06.lightgreen_cor.png',w=500,h=400)
verboseScatterplot(abs(geneModuleMembership[moduleGenes,module_column]),
                   abs(geneTraitCor[moduleGenes, 1]),
                   xlab = paste("Module Membership in", module, "module"),
                   ylab = "Gene significance for DEimmune Cell",
...
modgene <- data.frame(rownames(rbind(purple_hub,lightgreen_hub)))
