...
dat.fpkm<-read.delim2('/data/nas1/luchunlin/project/LLZK-505/00_rawdata/dat.fpkm.xls',row.names = 1)%>%lc.tableToNum()
colnames<-data.frame(sample=colnames(dat.fpkm))
colnames$sample<-gsub('.','-',colnames$sample,fixed = T)
colnames(dat.fpkm)<-colnames$sample
## 07-1 GRIM-19  NDUFS3------
### 07-1-1 差异分析-----
...
library(DESeq2)
colData<-data.frame(sample=rownames(cluster1),
                    group=cluster1$cluster)
...
write.table(DEG_write, file = "DEG_all(GRIM-19&NDUFS3).xls",
            quote = F,
            sep = "\t",
            row.names = F)
sig_diff_write <- cbind(GeneSymbol=rownames(sig_diff), sig_diff)
write.table(sig_diff_write, file = "DEG_sig(GRIM-19&NDUFS3).xls",
            quote = F,
            sep = "\t",
            row.names = F)
### 火山图---------
...
dat_rep<-DEG[rownames(DEG)%in%
               c(head(rownames(subset(sig_diff,sig_diff$log2FoldChange>1.06)),10),
                 head(rownames(subset(sig_diff,sig_diff$log2FoldChange< -1.1)),10)),]
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(Cluster1="lightblue",Cluster2="darkorange"),
...
ego <- enrichGO(gene = gene_transform$ENTREZID,
                ...
                qvalueCutoff = 2,
                readable = TRUE)
write.table(ego,file = "GO(GRIM-19&NDUFS3).xls",sep = "\t",quote = F,row.names = F)
# 展示富集最显著的 GO term
...
write.table(kk@result,file = "KEGG(GRIM-19&NDUFS3).xls",sep = "\t",quote = F,row.names = F)
...
colData<-data.frame(sample=rownames(cluster2),
                    group=cluster2$cluster)
colData$group<-factor(colData$group,levels = c('Cluster1','Cluster2'))
dds<-DESeqDataSetFromMatrix(countData = dat,colData=colData,design = ~group)
dds = dds[rownames(counts(dds)) > 1,]
dds<-DESeq(dds)
...
## 35
DEG_write <- cbind(GeneSymbol=rownames(DEG), DEG)
write.table(DEG_write, file = "DEG_all(NDUFA4&LRPPRC).xls",
            quote = F,
            sep = "\t",
            row.names = F)
sig_diff_write <- cbind(GeneSymbol=rownames(sig_diff), sig_diff)
write.table(sig_diff_write, file = "DEG_sig(NDUFA4&LRPPRC).xls",
            quote = F,
            sep = "\t",
            row.names = F)
### 火山图---------
...
dat_rep<-DEG[rownames(DEG)%in%
               c(head(rownames(subset(sig_diff,sig_diff$log2FoldChange>1.16)),10),
                 head(rownames(subset(sig_diff,sig_diff$log2FoldChange< -1.1)),10)),]
volcano_plot<- ggplot(data = DEG, 
                      aes(x = log2FoldChange,
                          y = -log10(padj), 
  ...
  labs(x = "log (Fold Change)",
       y = "-log10 (adj.P.Val)")
volcano_plot
ggsave('volcano(NDUFA4&LRPPRC).png', volcano_plot,width = 8, height = 7)
ggsave('volcano(NDUFA4&LRPPRC).pdf', volcano_plot,width = 8, height = 7)
### 热图--------
library(pheatmap)
library(ggplot2)
library(RColorBrewer)
...
ego <- enrichGO(gene = gene_transform$ENTREZID,
                ...
                qvalueCutoff = 2,
                readable = TRUE)
write.table(ego,file = "GO(NDUFA4&LRPPRC).xls",sep = "\t",quote = F,row.names = F)
# 展示富集最显著的 GO term
go_bar <- barplot(ego, showCategory=5, split="ONTOLOGY") +
  facet_grid(ONTOLOGY ~ ., scales = "free")
go_bar
...
kk <- enrichKEGG(gene = gene_transform$ENTREZID,
                 ...
                 qvalueCutoff = 2)
kk <- setReadable(kk, OrgDb = org.Hs.eg.db, keyType="ENTREZID")
write.table(kk@result,file = "KEGG(NDUFA4&LRPPRC).xls",sep = "\t",quote = F,row.names = F)
kk_dot <- dotplot(kk, showCategory=20)
kk_dot
