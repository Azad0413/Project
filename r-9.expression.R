...
control.sample <- group$sample[which(group$group=='control')]
## 样本分组
hub_exp2$Group<-ifelse(hub_exp2$sample%in%control.sample,'control','UC')

hub_exp2$Group <- factor(hub_exp2$Group,levels = c('UC','control'))
##分面图形
library(ggplot2)
library(tidyverse)
library(ggpubr)
...
ggsave('01.expression(train).png',exp_plot,width = 6,height = 6)


## 训练集
dat.va<-read.delim2('../00_rawdata/dat(GSE94648).xls',row.names = 1)%>%lc.tableToNum()
dat.va <- na.omit(dat.va)
# dat.va <- log2(dat.va+1)
group.va <- read.delim2('../00_rawdata/group(GSE94648).xls')
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  facet_wrap(~Symbol,scales = "free",nrow = 2) 
exp_plot
ggsave('02.expression(validation).pdf',exp_plot,width = 5,height = 5)
ggsave('02.expression(validation).png',exp_plot,width = 6,height = 6)
table(group$group)
## 趋势一致的3个
