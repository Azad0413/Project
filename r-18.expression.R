...
table(group$group)
modgene <- read.delim2('../08_Lasso/lasso_genes.csv',header = F)
hub_exp<-dat[modgene$V1,]
...
diff <- read.delim2('../01_DEG/DEG_sig.xls',row.names = 1)
...
exp_plot <- ggplot(hub_exp2,aes(x = Group, y = expr, color = Group)) +
  ...
  stat_pvalue_manual(stat.test,
                     y.position = c(8,10,11,10,5,11,3),
                     size = 3.2,
                     family = "Times",
                     label = "p.adj",
                     #parse = T,
