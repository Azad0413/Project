...
geneset$Efferocytosis<- c(rep('Efferocytosis',75))
...
score = gsva(dat2, gene_list,
             ...
             verbose = TRUE)
write.table(score,
            file = "Efferocytosis_score.xls",
            sep = "\t",
            quote = F)
Efferocytosis_score<-t(score)%>%as.data.frame()
...
# 1320 3623 3247 1138 1126  616  595  398  313  247  243  169   63 
## 04-4 层级聚类数展示各个模块
## 灰色的为**未分类**到模块的基因。
...
## 04-6 关联表型数据------
group_traits<-Efferocytosis_score
...
labeledHeatmap(Matrix = as.matrix(moduleTraitCor[-13,]),
               ...
               cex.text = 0.7,
               zlim = c(-1,1),
               main = paste("Module-Efferocytosis relationships"))
