...
library(lance)
library(limma)
df = read.delim2("../00_rawdata/dat(GSE97537).xls", row.names = 1) %>% lc.tableToNum%>%log2()
df.group = read.delim2("../00_rawdata/group(GSE97537).xls")
table(df.group$group)
df = df[df.group$sample]
df.group$group = factor(df.group$group, levels = c("control", "CIRI"))
design.mat = cbind(control = ifelse(df.group$group == "control", 1, 0), 
                   CIRI = ifelse(df.group$group == "control", 0, 1))

contrast.mat = makeContrasts(contrasts="CIRI-control", levels=design.mat)
...
# DOWN  NOT   UP 
# 238    0  676 
write.table(DEG,file = "DEG_all(GSE97537).xls",quote = F,sep = "\t",row.names = T)
write.table(sig_diff,file = "DEG_sig(GSE97537).xls",quote = F,sep = "\t",row.names = T)
## 火山图------
library(ggplot2)
library(ggthemes)
...
volcano_plot<- ggplot(data = DEG, 
  ...
  labs(x = "log (Fold Change)",
       y = "-log10 (adj.P.Val)")
volcano_plot
ggsave('01.volcano.png', volcano_plot,width = 7, height =5.5)
ggsave('01.volcano.pdf', volcano_plot,width = 7, height = 5.5)
## 热图-------
library(pheatmap)
library(ggplot2)
library(RColorBrewer)
...
#x<-t(scale(t(heat)))
ann_colors<-list(
  group = c(control="lightblue",CIRI="darkorange"),
